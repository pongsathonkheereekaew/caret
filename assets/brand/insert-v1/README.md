...
Refines the user's selected concept 02 into editable SVG. Approved as Cedia's
...
- `cedia.iconset` / `cedia.icns`: standard 16–1024px macOS icon representations.
  `bun run build:mac` copies the ICNS into `desktop/resources/darwin/code.icns`,
  the retained Electron and DMG packaging input, and `dist/brand/cedia.icns`.
  ...
  onto white. Finish with `iconutil -c icns cedia.iconset -o cedia.icns` from
  this directory.
...
`bun scripts/build-cedia.ts --desktop` and `--package` re-apply the ICNS to the
dev Electron bundle (`desktop/.build/electron/Cedia.app`, what

...
import { CediaHostClient } from "../apps/macos/src/api.ts";
import { fileSha256 } from "./lib/omp-runtime-integrity.ts";
const root = resolve(import.meta.dir, "..");
const fixture = await mkdtemp(join(tmpdir(), "cedia-portable-"));
...
try {
  ...
  const env = {
    ...
    TERM: "xterm-256color",
    CEDIA_RPC_EDITOR_BRIDGE: "1",
  };
  execFileSync(node, [cli, "ensure"], { cwd: workspace, env, timeout: 20000, stdio: "pipe" });
  client = await CediaHostClient.fromStateDir(stateDir, { timeoutMs: 25000 }); hostPid = client.descriptor.pid;

 ...
 *       Diff a captured Cedia tree against the reference. Non-zero exit while
 *       any control the reference has is missing from Cedia.
 *
 *   bun scripts/agents-chrome-inventory.ts capture <debugPort> [--write <file>] [--empty-draft]
 *       Read the running Cedia window over CDP, print its inventory, diff it.
 ...
 *
 * The reference capture is the same kind of tree the Cedia side can be captured
...
function report(reference: readonly ChromeControl[], cedia: readonly ChromeControl[]): number {
	...
	process.stdout.write(`reference controls: ${reference.length}, cedia controls: ${cedia.length}\n`);
	process.stdout.write(`shared: ${diff.shared.length}, renamed: ${diff.renamed.length}, absent by decision: ${diff.deviations.length}\n`);
	write("missing from Cedia (parity failures)", inventory(diff.missing));
	write("same control, different words", diff.renamed.map(pair => `${pair.reference.label}  <=>  ${pair.cedia.label}`));
	write("absent by decision", deviationReasons(diff.deviations));
	write("Cedia-only (informational, section 4)", inventory(diff.extra));
...
if (command === "reference") {
	...
	const cedia = chromeControlsFromTree(dump);
	...
	if (writeAt > 0) await Bun.write(process.argv[writeAt + 1]!, `${formatInventory(cedia)}\n`);
	process.exitCode = report(reference, cedia);
} else {

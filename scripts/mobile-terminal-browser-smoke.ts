...
const page = `<!doctype html><html><head><meta charset="utf-8"><title>Cedia terminal browser fixture</title></head><body>
<h1>Cedia terminal browser fixture</h1><p>No host connection or model calls.</p>
...
<iframe title="Cedia terminal" id="terminal" src="/terminal" sandbox="allow-scripts" style="display:block;width:850px;height:400px;border:1px solid #999"></iframe>
...
function send(payload){frame.contentWindow.postMessage(JSON.stringify({source:'cedia-terminal',payload}),'*')}
...
const server = Bun.serve({ hostname: "127.0.0.1", port: 0, fetch(request) {
  return new Response(new URL(request.url).pathname === "/terminal" ? terminalDocument({terminalId:"browser-fixture",cols:80,rows:20,title:"Cedia terminal fixture"}) : page, { headers: { "content-type": "text/html; charset=utf-8", "cache-control": "no-store" } });
}});
console.log(`Cedia terminal browser fixture: http://127.0.0.1:${server.port}`);

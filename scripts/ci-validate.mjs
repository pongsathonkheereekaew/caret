...
const PLAN = join(ROOT, 'docs', 'maintenance', 'CEDIA-PLAN.md');
check(existsSync(PLAN), 'plan: docs/maintenance/CEDIA-PLAN.md is missing');
const plan = readFileSync(PLAN, 'utf8');
for (const entry of readdirSync(evidenceDir, { withFileTypes: true })) {
  if (entry.isDirectory() && !plan.includes(entry.name)) {
    failures.push(`evidence/${entry.name}: not indexed in CEDIA-PLAN.md`);
  }
}
const otherPlans = markdown
  .map((f) => relative(ROOT, f))
  .filter((f) => /(^|\/)(PLAN|SPEC|ROADMAP|PARITY-MATRIX|UI-SPEC)[^/]*\.md$/i.test(f) && f !== 'docs/maintenance/CEDIA-PLAN.md');
check(
  otherPlans.length === 0,

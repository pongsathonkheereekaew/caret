/** Real OMP + CaretHost permission gate, using only a loopback scripted model. */
...
const dir = await mkdtemp(join(tmpdir(), "caret-native-permission-"));
...
try {
  ...
  for (const [name, payload] of [["set_model", { provider: "caret-fixture", modelId: "native-fixture" }], ["set_auto_retry", { enabled: false }], ["set_auto_compaction", { enabled: false }]] as const) {

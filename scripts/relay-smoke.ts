...
import { CaretRelayServer } from "../packages/relay/src/server.ts";
import { CaretRelayClient, createCaretRelayPairingOffer, generateKeyPair, exportPublicKey } from "../packages/relay/src/index.ts";
import type { CaretRelaySocket } from "../packages/relay/src/transport.ts";

const key = generateKeyPair();
const token = randomBytes(32).toString("base64url");
const offer = createCaretRelayPairingOffer({ serverId: randomUUID(), daemonPublicKeyB64: exportPublicKey(key.publicKey), relayEndpoint: "relay.paseo.sh:443", relayUseTls: true, deviceId: "synthetic-interop", deviceToken: token });
let revoked = false;
let effects = 0;
const server = new CaretRelayServer({ endpoint: offer.relayEndpoint, useTls: true, serverId: offer.serverId, daemonKeyPair: key, autoReconnect: false,
  ...
  handler: async request => {
    ...
    return { status: 200, body: { fixture: "caret-synthetic-interop", effects } };
  },
});
const client = new CaretRelayClient({ offer, createWebSocket: url => new WebSocket(url) as unknown as CaretRelaySocket, requestTimeoutMs: 10_000 });

...
export function buildOmpNative(source: string, sourceTree: string): string {
  if (process.platform !== "darwin") throw new Error("Caret's native runtime build currently targets macOS");
  ...
  const useIsolated = !process.env.CARET_CARGO && existsSync(join(isolatedCargo, "bin/cargo"));
  const cargo = process.env.CARET_CARGO ?? (useIsolated ? join(isolatedCargo, "bin/cargo") : "cargo");
  ...
  execFileSync(process.execPath, ["-e", "const n=require(process.argv[1]); if(typeof n.EditSession.prototype.setFileOverlay!=='function'||typeof n.EditSession.prototype.clearFileOverlays!=='function') throw Error('Missing Caret native editor overlay ABI')", target], { cwd: source, stdio: "inherit" });

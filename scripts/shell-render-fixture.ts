/** Local render fixture for the real Cedia Agents shell.
 *
...
const NONCE = "cedia-shell-fixture";

const now = "2026-09-14T00:00:00.000Z";

const project = {
	id: "p-demo",
	path: "/tmp/cedia-ui-review-20260914/workspace",
	name: "demo-repo",
	pinned: false,
	archived: false,
	createdAt: now,
...
const projects = [
	project,
	{ id: "p-api", path: "/tmp/cedia-ui-review-20260914/api", name: "cedia-api", pinned: false, archived: false, createdAt: now },
];
		...
		state.uiRequests = [
			{
				...
				request: {
					...
					id: "req-1",
					title: "Run command",
					message: "Run `bun test src/sync` in /tmp/cedia-ui-review-20260914/workspace?",
					scopes: ["bash"],
					dangerous: false,
				},
				sessionId: "s-1",
...
window.__cediaPosted = [];
window.acquireVsCodeApi = function () {
  return { postMessage: function (message) { window.__cediaPosted.push(message); }, getState: function () { return undefined; }, setState: function () {} };
};
window.__cediaFixture = ${fixture};
</script>`;
	const boot = `<script nonce="${NONCE}">
setTimeout(function () { window.postMessage({ type: "state", state: window.__cediaFixture }, "*"); }, 0);
</script>`;
...
console.log(`Cedia shell render fixture: http://127.0.0.1:${server.port}/?fixture=empty`);

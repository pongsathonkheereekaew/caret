/**
 * Real subprocess acceptance probe for Caret's opt-in OMP virtual UI patch.
 *
 ...
 * modifies a user Caret/OMP session. The receipt contains only bounded frame
...
const requestedBinary = process.env.CARET_OMP_BINARY ?? join(root, "dist/omp/omp");
const executable = resolve(requestedBinary);
const fixture = join(root, "packages/omp-adapter/test/fixtures/caret-virtual-ui-extension.ts");
...
function strictEnv(cwd: string, virtual = true, autoReleaseMs?: number): NodeJS.ProcessEnv {
	return {
		...
		TERM: "xterm-256color",
		...(virtual ? { CARET_RPC_VIRTUAL_UI: "1" } : {}),
		...(autoReleaseMs === undefined ? {} : { CARET_VIRTUAL_UI_AUTO_RELEASE_MS: String(autoReleaseMs) }),
	};
}

function frameSummary(frame: OmpFrame): Record<string, unknown> {
	const type = textValue(frame.type);
	if (type === "caret_terminal_output") {
		...
		return {
			...
			markers: [
				"caret-virtual-startup",
				"caret-virtual-command",
				"caret-virtual-pty-output",
	...
	if (type === "caret_terminal_open" || type === "caret_terminal_close") {
...
class FixtureModelServer {
	...
	async #handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
		try {
			...
			check(payload.model === "caret-scripted-model", "Unexpected fixture model");
			...
			const call = this.turn === "pty"
				? { command: "printf 'caret-virtual-pty-output\\n'", pty: true }
				: { command: "sleep 30", pty: true };
			const chunk = (delta: unknown, finishReason: string | null) => ({
				id: `caret-virtual-ui-fixture-${this.totalRequests}`,
				object: "chat.completion.chunk",
				created: 0,
				model: "caret-scripted-model",
				choices: [{ index: 0, delta, finish_reason: finishReason }],
			});
			const events = useTool
				? [
						chunk({ role: "assistant", tool_calls: [{ index: 0, id: `caret-virtual-call-${this.totalRequests}`, type: "function", function: { name: "bash", arguments: JSON.stringify(call) } }] }, null),
						chunk({}, "tool_calls"),
					]
				: [chunk({ role: "assistant", content: "Caret virtual UI loopback completed." }, "stop")];
...
async function waitForOutput(frames: OmpFrame[], marker: string, label: string): Promise<OmpFrame> {
	return waitForFrame(
		frames,
		frame => frame.type === "caret_terminal_output" && textValue(frame.data).includes(marker),
		label,
	);
}
 ...
 * is intentionally limited to the startup ordering probe: Caret must tolerate
...
async function probePromptBeforeNegotiation(cwd: string, extension: string, checks: string[]): Promise<void> {
	...
	try {
		...
		sendRawCommand(child, {
			id: "caret-pre-negotiate-prompt",
			type: "prompt",
			message: "this prompt must not enter the model before virtual UI startup",
		});
		sendRawCommand(child, {
			id: "caret-pre-negotiate-protocol",
			type: "negotiate_protocol",
			protocolVersion: 2,
		});
		const prompt = await waitForRawFrame(frames, frame => frame.id === "caret-pre-negotiate-prompt", "pre-negotiation prompt response");
		check(prompt.success === false && prompt.code === "caret_initializing", `Pre-negotiation prompt outcome was ${JSON.stringify(prompt)}`);
		const protocol = await waitForRawFrame(frames, frame => frame.id === "caret-pre-negotiate-protocol", "raw protocol negotiation response");
		...
		sendRawCommand(child, {
			id: "caret-pre-negotiate-terminal",
			type: "caret_terminal_negotiate",
			payload: { version: 1, cols: 40, rows: 8 },
		});
		const open = await waitForRawFrame(frames, frame => frame.type === "caret_terminal_open", "raw virtual terminal open");
		const terminalId = textValue(open.terminalId);
		check(terminalId.length > 0, "Raw virtual terminal open omitted terminalId");
		await waitForRawFrame(frames, frame => frame.type === "caret_terminal_output" && textValue(frame.data).includes("caret-virtual-startup"), "raw startup custom output");
		sendRawCommand(child, {
			id: "caret-pre-negotiate-input",
			type: "caret_terminal_input",
			payload: { terminalId, data: "caret-virtual-startup-ok" },
		});
		const input = await waitForRawFrame(frames, frame => frame.id === "caret-pre-negotiate-input", "raw startup input response");
		check(input.success === true, `Raw startup input failed: ${JSON.stringify(input)}`);
		await waitForRawFrame(
			frames,
			frame => frame.type === "extension_ui_request" && frame.method === "notify" && frame.message === "caret-virtual-startup-complete",
...
async function probeVirtualHandshake(cwd: string, extension: string, checks: string[]): Promise<void> {
	...
	try {
		check(client.readyFrame?.caretVirtualUiVersion === 1, "Patched OMP did not advertise Caret virtual UI v1");
		checks.push("subprocess-ready-advertises-virtual-ui-v1");
		const negotiated = await client.requestCaret("caret_terminal_negotiate", { version: 1, cols: 40, rows: 8 });
		const terminalId = frameTerminalId(await waitForFrame(frames, frame => frame.type === "caret_terminal_open", "virtual terminal open"));
		...
		await waitForOutput(frames, "caret-virtual-startup", "session_start custom output");
		await client.requestCaret("caret_terminal_input", { terminalId, data: "caret-virtual-startup-ok" });
		await waitForFrame(frames, frame => frame.type === "extension_ui_request" && frame.method === "notify" && frame.message === "caret-virtual-startup-complete", "session_start custom resolves");
		checks.push("session-start-custom-waits-for-negotiated-input");
		await waitForOutput(frames, "caret-virtual-editor", "mounted virtual CustomEditor draft");
		await client.requestCaret("caret_terminal_input", { terminalId, data: "!" });
		await waitForFrame(
			frames,
			frame =>
				frame.type === "extension_ui_request" &&
				frame.method === "notify" &&
				frame.message === "caret-virtual-editor-text:caret-virtual-editor!",
		...
		check(Array.isArray(commands) && commands.some(command => record(command).name === "caret-virtual-ui-command"), "Trusted virtual fixture command was not loaded");
		const ack = await client.request("prompt", { message: "/caret-virtual-ui-command" });
		check(typeof ack.id === "string", "Prompt ACK did not include a correlation id");
		await waitForOutput(frames, "caret-virtual-command", "slash command custom output");
		await client.requestCaret("caret_terminal_input", { terminalId, data: "caret-virtual-command-ok" });
		...
		await waitForFrame(frames, frame => frame.type === "extension_ui_request" && frame.method === "notify" && frame.message === "caret-virtual-command-complete", "slash command custom completion");
...
async function probeFirstRunWithoutModel(extension: string, checks: string[]): Promise<void> {
	const cwd = await mkdtemp(join(tmpdir(), "caret-first-run-"));
	...
	try {
		...
		await client.requestCaret("caret_terminal_negotiate", { version: 1, cols: 80, rows: 24 });
		const terminalId = frameTerminalId(await waitForFrame(frames, frame => frame.type === "caret_terminal_open", "first-run terminal"));
		await waitForOutput(frames, "caret-virtual-startup", "first-run startup UI");
		await client.requestCaret("caret_terminal_input", { terminalId, data: "caret-virtual-startup-ok" });
		await waitForFrame(frames, frame => frame.type === "extension_ui_request" && frame.message === "caret-virtual-startup-complete", "first-run initialized");
...
async function probePtyAndAbort(cwd: string, extension: string, model: FixtureModelServer, checks: string[]): Promise<void> {
	...
	try {
		const negotiated = await client.requestCaret("caret_terminal_negotiate", { version: 1, cols: 40, rows: 8 });
		const terminalId = frameTerminalId(await waitForFrame(frames, frame => frame.type === "caret_terminal_open", "PTY probe virtual terminal open"));
		check(record(negotiated.data).version === 1, "PTY probe virtual terminal negotiate failed");
		await waitForOutput(frames, "caret-virtual-startup", "PTY probe session_start custom output");
		await client.requestCaret("caret_terminal_input", { terminalId, data: "caret-virtual-startup-ok" });
		await waitForFrame(frames, frame => frame.type === "extension_ui_request" && frame.method === "notify" && frame.message === "caret-virtual-startup-complete", "PTY probe session_start custom completion");
		...
		const terminalOutputBeforePty = frames.filter(frame => frame.type === "caret_terminal_output").length;
		...
		check(JSON.stringify(ptyContent ?? "").includes("caret-virtual-pty-output"), "PTY fixture output was not returned");
		check(frames.filter(frame => frame.type === "caret_terminal_output").length > terminalOutputBeforePty, "PTY bash did not render through the negotiated virtual terminal");
...
async function probeEofClose(cwd: string, extension: string, checks: string[]): Promise<void> {
	...
	try {
		...
		await delay(20);
		sendRawCommand(child, {
			id: "caret-eof-protocol",
			type: "negotiate_protocol",
			protocolVersion: 2,
		});
		const protocol = await waitForRawFrame(frames, frame => frame.id === "caret-eof-protocol", "EOF probe protocol negotiation response");
		check(protocol.success === true, `EOF probe protocol negotiation failed: ${JSON.stringify(protocol)}`);
		sendRawCommand(child, {
			id: "caret-eof-terminal",
			type: "caret_terminal_negotiate",
			payload: { version: 1, cols: 40, rows: 8 },
		});
		const open = await waitForRawFrame(frames, frame => frame.type === "caret_terminal_open", "EOF probe virtual terminal open");
		const terminalId = textValue(open.terminalId);
		check(terminalId.length > 0, "EOF probe virtual terminal open omitted terminalId");
		await waitForRawFrame(frames, frame => frame.type === "caret_terminal_output" && textValue(frame.data).includes("caret-virtual-startup"), "EOF probe startup custom output");
		...
		check(
			frames.some(frame => frame.type === "caret_terminal_close" && frame.reason === "rpc_client_disconnected"),
...
async function probeHostBootstrap(cwd: string, extension: string, checks: string[]): Promise<void> {
	const stateDir = await mkdtemp(join(tmpdir(), "caret-virtual-host-"));
	let store: DurableStore | undefined;
	let host: CaretHost | undefined;
	try {
		store = DurableStore.open({ stateDir });
		const project = store.createProject({ path: cwd, name: "Caret virtual UI fixture" });
		...
		const open = await waitForFrame(events, frame => frame.type === "caret_terminal_open", "host virtual terminal open");
		...
		const input = await host.command(created.id, "virtual-probe-device", {
			...
			command: "caret_terminal_input",
			payload: { terminalId, data: "caret-virtual-startup-ok" },
		});
		check(input.status === "completed", `Host terminal input command was ${input.status}`);
		const started = await withTimeout(start, 5_000, "CaretHost startSession bootstrap");
		check(started.id === created.id, "CaretHost started a different session");
		check(resolved, "CaretHost startSession did not resolve");
...
async function main(): Promise<void> {
	...
	const cwd = await mkdtemp(join(tmpdir(), "caret-virtual-ui-"));
	...
	try {
		...
		const receipt = {
			...
			checks,
			limitations: "No external provider inference, visual Mac/iPhone rendering, cellular transport, signing, or production editor bridge. The host bootstrap check records whether startSession can complete while a session_start custom UI is pending; input is sent through CaretHost's durable command path.",
		};
		const output = process.env.CARET_SMOKE_RECEIPT;
		if (output) {

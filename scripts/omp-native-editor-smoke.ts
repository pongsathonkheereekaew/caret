...
const dir = await realpath(await mkdtemp(join(tmpdir(), "cedia-native-edit-")));
...
const host = new CediaHost({ store, stateDir, editors, ompExecutable: executable, editorBridge: true,
...
try {
  ...
  for (const [name, payload] of [["set_model", { provider: "cedia-fixture", modelId: "editor-fixture" }], ["set_auto_retry", { enabled: false }], ["set_auto_compaction", { enabled: false }]] as const) check((await command(name, payload)).status === "completed", `Setup ${name} failed`);

/** Real-app acceptance fixture for Cedia's agent-edit marks.
 *
 * The flagship Cursor-class affordance is: OMP applies an edit through the
 * guarded editor bridge, Cedia marks the region it produced *in the real
 ...
 * Cedia app against a private profile. No paid inference: the model is a
 * local scripted server.
 *
 * Usage:
 *   CEDIA_OMP_BINARY=<abs omp> bun scripts/ide-native-agent-mark-smoke.ts [--app <path>]
 *
 * Stages are operator/CUA-gated with marker files in the fixture dir:
 *   ready     - operator has the workspace + fixture file open in Cedia
 *   reverted  - operator ran "Cedia: Take Back the Cedia Edit"
 */
...
const app = appIndex >= 0
	...
	: join(root, `VSCode-darwin-${process.arch}`, "Cedia.app", "Contents", "MacOS", "Cedia");
...
const logical = "/tmp/cedia-agent-mark";
...
execFileSync("git", ["-c", "user.email=cedia@example.test", "-c", "user.name=Cedia", "commit", "-qm", "baseline"], { cwd: workspace });
...
try {
	...
	for (const [name, payload] of [
		["set_model", { provider: "cedia-fixture", modelId: "mark-fixture" }],
	...
	const run = async (message: string): Promise<void> => {
		...
		check(await readFile(path, "utf8") === baseline, "Cedia wrote behind the editor: the file on disk changed");
	};

	launch([workspace]);
	console.log(JSON.stringify({ stage: "prepare", fixture: dir, workspace, stateDir, profile, app, path,
		instructions: "Wait for Cedia to open the fixture workspace, then open src/greet.ts so a mark can be painted. Create the 'ready' marker." }));
	await waitFor("the Cedia editor connection", () => server.editors.hasConnection(workspace), 120_000);
	launch(["--goto", `${path}:1`]);
	...
	console.log(JSON.stringify({ stage: "verify-mark", fixture: dir,
		instructions: "The editor now holds the Cedia-applied edit (unsaved). Optionally run 'Cedia: Review the Cedia Edit'. Then EITHER run 'Cedia: Keep the Cedia Edit' and create the 'kept' marker, OR run 'Cedia: Take Back the Cedia Edit' and create the 'reverted' marker." }));

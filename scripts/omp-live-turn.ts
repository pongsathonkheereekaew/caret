/**
 * One real OMP turn through the Caret host API - the S2 end-to-end receipt.
 *
...
const EXPECTED = "caret-live-turn-ok";
...
const descriptorPath = process.env.CARET_HOST_DESCRIPTOR ?? join(homedir(), "Library/Application Support/Caret/host/host.json");
...
const projectPath = resolve(process.env.CARET_LIVE_TURN_CWD ?? ROOT);
...
const created = record(await call("POST", "/sessions", { projectId: project.id, title: "Caret live turn check" }));

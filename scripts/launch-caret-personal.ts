...
import { personalCaretLaunchArgs } from "./lib/personal-argv.ts";

const root = join(import.meta.dir, "..");
const app = join(root, `VSCode-darwin-${process.arch}`, "Caret.app", "Contents", "MacOS", "Caret");
 ...
 *
 * Only the personal launcher does this, and only for Caret's own item: the
...
function clearStaleChromiumSafeStorage(): void {
	const service = "Caret Safe Storage";
	try {
...
if (!existsSync(app)) {
	process.stderr.write(`Caret.app is not packaged at ${app}. Build with --package first.\n`);
	process.exitCode = 1;
} else {
	clearStaleChromiumSafeStorage();
	const child = spawn(app, [...personalCaretLaunchArgs()], { detached: true, stdio: "ignore" });
	child.unref();
	process.stdout.write(`Started personal Caret with ${personalCaretLaunchArgs().join(" ")}\n`);
}

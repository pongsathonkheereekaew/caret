...
export default function (api: any) {
	...
	api.on("session_start", async (_event: unknown, ctx: any) => {
		await ctx.ui.custom((_tui: unknown, _theme: unknown, _keys: unknown, done: (value: string) => void) =>
			component("caret-virtual-startup", done),
		);
		...
		ctx.ui.setEditorText("caret-virtual-editor");
		ctx.ui.onTerminalInput((data: string) => {
			if (data === "!") {
				queueMicrotask(() => ctx.ui.notify(`caret-virtual-editor-text:${ctx.ui.getEditorText()}`, "info"));
			}
		});
		ctx.ui.notify("caret-virtual-startup-complete", "info");
	});

	api.registerCommand("caret-virtual-ui-command", {
		description: "Deterministic Caret virtual UI command",
		handler: async (_args: string, ctx: any) => {
			await ctx.ui.custom((_tui: unknown, _theme: unknown, _keys: unknown, done: (value: string) => void) =>
				component("caret-virtual-command", done),
			);
			ctx.ui.notify("caret-virtual-command-complete", "info");
		},
	});
	...
	api.registerTool({
		name: "bash",
		label: "Bash (Caret fixture wrapper)",
		description: "Caret fixture wrapper around OMP's native bash",
		parameters: {
			type: "object",
			properties: {
				command: { type: "string" },
		...
		execute: async (
			...
			return { content: [{ type: "text", text: "caret fixture invokeTool unavailable" }], details: { unavailable: true } };
		},
	});
}

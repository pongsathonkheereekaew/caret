...
describe("OMP runtime version policy", () => {
	/**
	 * The baseline is a floor, not a pin: Caret follows OMP, so a machine whose runtime is
	 * newer than the release the contract was written against runs normally, and what that
	 * runtime supports is read from its own `ready` frame (the Caret bridges are
	...
	it("accepts a newer major line", () => {
		// A version string cannot tell Caret what a runtime does; the ready frame can, and the

...
export interface CediaRelaySocket {
...
export type CediaRelayWebSocketFactory = (url: string) => CediaRelaySocket;

export interface CediaRelaySocketMessage {
	data: string | ArrayBuffer | Uint8Array;
	isBinary: boolean;
}

export interface CediaRelayTransport {
	send(data: string | ArrayBuffer): void | Promise<void>;
	close(code?: number, reason?: string): void;
	onmessage: ((message: CediaRelaySocketMessage) => void) | null;
...
export function createSocketTransport(socket: CediaRelaySocket): CediaRelayTransport {
	try {
		socket.binaryType = "arraybuffer";
	} catch {
		// React Native sockets may not expose binaryType.
	}
	const transport: CediaRelayTransport = {
		send(data) {

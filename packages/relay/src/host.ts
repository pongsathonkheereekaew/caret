/**
 * Caret request endpoint running on the Mac side of an encrypted relay
...
import {
	...
	type CaretRelayHandlerRequest,
	type CaretRelayHandlerResponse,
	type CaretRelayJson,
	type CaretRelayRequest,
...
export type CaretRelayRequestHandler = (
	request: CaretRelayHandlerRequest,
	signal?: AbortSignal,
) => CaretRelayHandlerResponse | Promise<CaretRelayHandlerResponse>;

export interface CaretRelayHostOptions {
...
interface RequestRecord {
	fingerprint: string;
	promise: Promise<CaretRelayResponse>;
	controller: AbortController;
	completed: boolean;
	response?: CaretRelayResponse;
}
...
function jsonError(code: string, message: string): CaretRelayJson {
	return { error: { code, message } };
}

function isResponse(value: unknown): value is CaretRelayResponse {
	return typeof value === "object" && value !== null && (value as { type?: unknown }).type === "response";
}

function responseFor(request: CaretRelayRequest, status: number, body: CaretRelayJson): CaretRelayResponse {
	return {
...
function isOversizedSerializationError(error: unknown): boolean {
	return error instanceof CaretRelayProtocolError && /relay (?:message|frame) exceeds \d+ bytes/i.test(error.message);
}
...
export class CaretRelayHost {
	readonly #channel: EncryptedChannel;
	readonly #handler: CaretRelayRequestHandler;
	readonly #authorize: (token: string) => boolean;
	readonly #maxRequestIds: number;
	readonly #maxPendingRequests: number;
	readonly #requestTimeoutMs: number;
	readonly #onError?: CaretRelayHostOptions["onError"];
	...
	constructor(options: CaretRelayHostOptions) {
	...
	async #receive(data: string | ArrayBuffer): Promise<void> {
		let parsed: ReturnType<typeof parseCaretRelayMessage>;
		try {
			parsed = parseCaretRelayMessage(data);
			if (isResponse(parsed)) throw new CaretRelayProtocolError("host received a response frame");
		} catch (error) {
		...
		if (parsed.epoch !== this.#epoch) {
			this.#protocolError(new CaretRelayProtocolError("relay request epoch changed on one connection"));
			return;
		}

		const fingerprint = fingerprintCaretRelayRequest(parsed);
		...
		if (previous) {
			if (previous.fingerprint !== fingerprint) {
				this.#protocolError(new CaretRelayProtocolError("relay request id was reused with a different payload"));
				return;
			}
		...
		if (this.#active.size >= this.#maxPendingRequests) {
			await this.#sendResponse(parsed, responseFor(parsed, 429, jsonError("host_capacity", "Caret host request capacity reached")));
			return;
		}
	...
	async #run(request: CaretRelayRequest, controller: AbortController, release: () => void): Promise<CaretRelayResponse> {
		const timeoutMarker = Symbol("timeout");
		const disposedMarker = Symbol("disposed");
		let timer: ReturnType<typeof setTimeout> | undefined;
		const handlerPromise: Promise<CaretRelayHandlerResponse> = Promise.resolve().then(async () => {
			if (this.#disposed) return { status: 503, body: jsonError("disposed", "Caret host is unavailable") };
		...
		try {
			...
			if (outcome === timeoutMarker) return responseFor(request, 504, jsonError("timeout", "Caret host request timed out"));
			if (outcome === disposedMarker) return responseFor(request, 503, jsonError("disposed", "Caret host is unavailable"));
			const result = outcome as CaretRelayHandlerResponse;
			if (!result || typeof result !== "object" || !Number.isSafeInteger(result.status) || result.status < 100 || result.status > 599) {
				return responseFor(request, 500, jsonError("invalid_handler_response", "Caret host returned an invalid response"));
			}
			if (controller.signal.aborted) return responseFor(request, 504, jsonError("timeout", "Caret host request timed out"));
			return responseFor(request, result.status, result.body);
		} catch (error) {
			this.#emit(asError(error), "handler");
			return responseFor(request, 500, jsonError("handler_error", "Caret host request failed"));
		} finally {
	...
	#fitResponse(request: CaretRelayRequest, response: CaretRelayResponse): CaretRelayResponse {
		try {
			serializeCaretRelayMessage(response);
			return response;
		} catch (error) {
			...
			return responseFor(
				...
				jsonError(
					...
					isOversizedSerializationError(error)
						? "Caret host response exceeds the relay payload limit"
						: "Caret host returned a response that cannot be serialized",
				),
			);
		}
	}
	...
	async #sendResponse(request: CaretRelayRequest, response: CaretRelayResponse): Promise<void> {
		if (this.#disposed || !this.#channel.isOpen()) return;
		...
		if (this.#disposed || !this.#channel.isOpen()) return;
		try {
			await this.#channel.send(serializeCaretRelayMessage(safeResponse));
		} catch (error) {

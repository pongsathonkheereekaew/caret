/**
 * Node host transport for a Cedia relay.  This is deliberately a separate
...
import { CediaRelayHost, type CediaRelayRequestHandler } from "./host.ts";
...
export interface CediaRelayServerOptions {
	endpoint: string;
	useTls: boolean;
	serverId: string;
...
function defaultWebSocket(url: string): CediaRelaySocket {
	return new NodeWebSocket(url, { handshakeTimeout: 10_000, perMessageDeflate: false }) as unknown as CediaRelaySocket;
}
...
export class CediaRelayServer {
	readonly #options: CediaRelayServerOptions;
	readonly #factory: CediaRelayWebSocketFactory;
	readonly #data = new Map<string, DataConnection>();
	#control: CediaRelaySocket | null = null;
	#started = false;
	#stopped = true;
	...
	constructor(options: CediaRelayServerOptions) {
	...
	#scheduleReconnect(): void {
		...
		let host: CediaRelayHost | undefined;
		...
		void channelPromise.then((channel) => {
			...
			host = connection.host = new CediaRelayHost({ channel, handler: async (request, context) => {

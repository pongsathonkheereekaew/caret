...
export function stepDeviceFrameGate(
  ...
  if (header.codecConfig) {
    return {
      ...
      // The helper always emits a keyframe immediately after its parameter
      // sets. A codec-config frame can therefore be the response to a resync
      // request; asking for another resync here would create an endless
      // configure → restart loop and the pane would remain on Connecting….
  ...
  switch (availability.kind) {
    ...
    case "unsupported-platform":
      return {
        ...
        description: `This Cedia server runs on ${availability.platform}. Simulators are only available when the server runs on a Mac with Xcode installed.`,
        steps: [],
        retryable: false,
      };
    case "setup-required":

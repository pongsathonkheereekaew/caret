...
export type SidebarUiState = {
  /** Whether the desktop thread sidebar was last left open. */
  sidebarOpen: boolean;
...
export type SidebarUiStatePersistInput = Omit<SidebarUiState, "sidebarOpen"> &
  Partial<Pick<SidebarUiState, "sidebarOpen">>;

export function persistSidebarUiState(input: SidebarUiStatePersistInput): void {
  if (typeof window === "undefined") {
    return;
  }

  try {
    // Older callers persist the rest of the sidebar state independently of the
    // shell open/closed toggle. Preserve the current value when they omit it so
    // a thread-list update can never reopen a sidebar the user just collapsed.
    const current = readSidebarUiState();
    window.localStorage.setItem(
      SIDEBAR_UI_STATE_STORAGE_KEY,
      JSON.stringify({
        sidebarOpen: input.sidebarOpen ?? current.sidebarOpen,

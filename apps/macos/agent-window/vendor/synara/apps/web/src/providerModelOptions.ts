...
// View-only state used before the native OMP adapter has advertised its live
// catalogue. It must never be sent as a model id or inserted into a catalog.
export const OMP_UNRESOLVED_MODEL = "cedia:unresolved" as const;

export function isOmpUnresolvedModel(model: string | null | undefined): boolean {
  return model === OMP_UNRESOLVED_MODEL;
}
...
export function formatProviderModelOptionName(input: {
  ...
  if (input.provider === "omp" && isOmpUnresolvedModel(trimmedSlug)) {
    return "Choose model";
  }

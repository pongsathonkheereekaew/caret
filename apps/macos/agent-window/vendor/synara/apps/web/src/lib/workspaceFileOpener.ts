...
/**
 * Resolves a chat file reference to the absolute path the IDE bridge needs.
 * The IDE dock mounts no file pane (RightDock only renders in the standalone
 * agent window), so embedded file opens bypass the dock store — whose write
 * would be invisible — and go straight to the native editor. Returns null when
 * the reference cannot be mapped to a path.
 */
export function resolveIdeFileOpenTarget(
  workspaceRoot: string | null,
  rawPath: string,
): string | null {
  const target = resolveDockFileOpenTarget(rawPath, workspaceRoot);
  if (!target) {
    return null;
  }
  ...
  if (!workspaceRoot || !isWorkspaceRelativePathSafe(target)) {
    return null;
  }
  return `${workspaceRoot.replace(/\/+$/, "")}/${target}`;
}

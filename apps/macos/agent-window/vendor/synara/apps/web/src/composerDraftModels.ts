...
export function normalizeProviderModelOptions(
  ...
  // OMP owns the model-specific ladder; keep its advertised value without
  // narrowing it to another harness's fixed set of effort levels.
  const ompThinkingLevel = trimStringOrUndefined(ompCandidate?.thinkingLevel);
  const omp = ompThinkingLevel ? { thinkingLevel: ompThinkingLevel } : undefined;
...
export function deriveEffectiveComposerModelState(input: {
  ...
  const selectedModel =
    ...
    baseModel ??
    getDefaultModel(input.selectedProvider) ??
    (input.selectedProvider === "omp" ? "cedia:unresolved" : getDefaultModel("codex"));
...
export function resolvePreferredComposerModelSelection(input: {
  ...
  return (
    draftSelection ??
    persistedSelection ?? {
      provider: preferredProvider === "pi" ? "omp" : preferredProvider,
      model:
        getDefaultModel(preferredProvider === "pi" ? "omp" : preferredProvider) ??
        "cedia:unresolved",
    }
  );
}

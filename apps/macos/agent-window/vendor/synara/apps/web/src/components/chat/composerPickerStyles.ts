...
/**
 * Origin-aware menu dropdown motion. Base UI keeps popup nodes mounted while the
 * transition status is `ending`, so the data-starting/data-ending selectors animate
 * both directions without a second presence wrapper. The transform origin comes from
 * the anchored positioner (`--transform-origin`), which keeps the panel attached to
 * the trigger as it scales in.
 */
export const COMPOSER_PICKER_MENU_DROPDOWN_MOTION_CLASS_NAME =
  "origin-(--transform-origin) will-change-[transform,opacity,filter] transition-[transform,opacity,filter] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] data-starting-style:scale-[0.97] data-starting-style:opacity-0 data-starting-style:blur-[2px] data-ending-style:scale-[0.99] data-ending-style:opacity-0 data-ending-style:blur-[1px] data-ending-style:duration-150 motion-reduce:transition-none";

/**
 * Keep the model menu's final layout throughout its reveal. Animating width and
 * max-height from the trigger made the anchored positioner chase every frame;
 * rounded-full also turned the intermediate surface into a circle/capsule.
 * A small origin-aware reveal preserves its rectangular shape and text layout.
 */
export const COMPOSER_PICKER_MENU_MORPH_MOTION_CLASS_NAME =
  "origin-(--transform-origin) max-h-[var(--available-height)] overflow-hidden rounded-[0.875rem] transition-[opacity,scale,translate] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] data-starting-style:opacity-0 data-starting-style:scale-[0.98] data-starting-style:translate-y-1 data-ending-style:opacity-0 data-ending-style:scale-[0.98] data-ending-style:translate-y-1 data-ending-style:duration-150 motion-reduce:transition-none";

/** Content remains in its final layout while the containing surface reveals. */
export const COMPOSER_PICKER_MENU_MORPH_CONTENT_CLASS_NAME = "min-w-0";
...
/**
 * Environment card reveal, adapted from transitions.dev's “Menu dropdown” (P2):
 * 250ms origin-aware open, 150ms close, and no blur so the dense card stays crisp.
 * The panel component supplies the open/closing scale states while this token keeps
 * timing and reduced-motion behavior in one place.
 */
export const ENVIRONMENT_PANEL_MOTION_CLASS =
  "origin-top-right will-change-[transform,opacity] transition-[transform,opacity] duration-250 ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none";

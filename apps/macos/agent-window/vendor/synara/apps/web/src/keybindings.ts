...
export const DEFAULT_SHORTCUT_FALLBACKS: ResolvedKeybindingsConfig = [
  // Search + theme palette (advertised as ⌘K in the sidebar search tooltip).
  // App-level `mod` chords like the other entries here: they fire from a focused
  // terminal on macOS (xterm never sees Cmd) and yield to the shell on
  // Linux/Windows via `whenModChordAllowed`. ⇧⌘P first so IDE muscle memory works;
  // ⌘K stays last so chrome affordances keep showing ⌘K (label resolution reads
  // bindings from the end).
  {

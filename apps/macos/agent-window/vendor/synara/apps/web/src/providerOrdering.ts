...
/** Returns a stable provider order after moving one provider before another. */
export function moveProviderInOrder(
  providerOrder: ReadonlyArray<ProviderKind>,
  active: ProviderKind,
  over: ProviderKind,
): ProviderKind[] {
  const fromIndex = providerOrder.indexOf(active);
  const toIndex = providerOrder.indexOf(over);
  if (fromIndex < 0 || toIndex < 0 || fromIndex === toIndex) return [...providerOrder];

...
/**
 * Show the meter before the first OMP turn when discovery already supplied the
 * model's context limit. Usage remains explicitly zero until OMP reports real
 * occupancy; this is a capacity indicator, not a fabricated token count.
 */
export function deriveRuntimeModelContextWindowSnapshot(runtimeModel: {
  readonly contextWindowOptions?: ReadonlyArray<{ readonly value?: string; readonly isDefault?: true }>;
} | null | undefined): ContextWindowSnapshot | null {
  const options = runtimeModel?.contextWindowOptions;
  if (!options?.length) return null;
  const preferred = options.find(option => option.isDefault === true) ?? options[0];
  const raw = preferred?.value?.trim().toLowerCase();
  if (!raw) return null;
  ...
  const maxTokens = Math.round(amount * multiplier);

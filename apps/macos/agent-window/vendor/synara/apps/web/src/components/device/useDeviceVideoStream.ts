...
export function useDeviceVideoStream(input: {
  ...
  useEffect(() => {
    ...
    const configureDecoder = (frame: DeviceFrame) => {
      ...
      if (!codec) {
        failStream("The simulator stream sent parameters Cedia could not read.");
        return;
      }
      teardownDecoder();
    ...
    const handleReset = (reason: DeviceFrameSourceResetReason, error?: unknown) => {
      if (!isCurrent() || disposed) return;
      teardownDecoder();
      gate = createDeviceFrameGateState();
      if (reason === "closed") {
      ...
      setStatus({
        ...
        message:
          reason === "decode-failed"
            ? "The simulator stream sent a frame Cedia could not read."
            : `The simulator stream disconnected.${detail}`,
      });
    };

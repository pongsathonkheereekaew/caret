...
import { copyTextToClipboard, useCopyPathToClipboard, useCopyThreadIdToClipboard } from "~/hooks/useCopyToClipboard";
import {
...
import {
  ...
  deriveRuntimeModelContextWindowSnapshot,
...
export default function ChatView({
  ...
  useEffect(() => {
    if (!isIdeEmbeddedRuntime()) return;
    const onIdeAction = (event: Event): void => {
      const detail = (event as CustomEvent<{ action?: unknown; text?: unknown }>).detail;
      if (!detail || typeof detail.action !== "string") return;
      if (detail.action === "focus") {
        scheduleComposerFocus();
        return;
      }
      if (detail.action === "newTask") {
        void handleNewChat();
        return;
      }
      if (detail.action !== "appendContext" || typeof detail.text !== "string") return;
      const text = detail.text.trim();
      if (!text) return;
      const currentPrompt = useComposerDraftStore.getState().draftsByThreadId[threadId]?.prompt ?? "";
      setComposerDraftPrompt(threadId, currentPrompt ? `${currentPrompt}\n\n${text}` : text);
      scheduleComposerFocus();
    };
    window.addEventListener("cedia:ide-action", onIdeAction);
    window.dispatchEvent(new Event("cedia:ide-ui-ready"));
    return () => window.removeEventListener("cedia:ide-action", onIdeAction);
  }, [handleNewChat, scheduleComposerFocus, setComposerDraftPrompt, threadId]);
  useEffect(() => {
    window.dispatchEvent(new Event("cedia:chat-ui-ready"));
  }, [threadId]);
        ...
        : (activeContextWindow ??
          (selectedProvider === "claudeAgent"
            ...
            : selectedProvider === "omp"
              ? deriveRuntimeModelContextWindowSnapshot(selectedRuntimeModel)
              : null)),
    [
      activeContextWindow,
      activeContextWindowState.invalidatedByCompaction,
  ...
  const toggleActiveThreadPin = useCallback(() => {
    ...
    void api.orchestration
      ...
      .catch((error) => {
        toastManager.add({
          type: "error",
          title: activeThread.isPinned ? "Unable to unpin thread" : "Unable to pin thread",
          description: error instanceof Error ? error.message : "The thread could not be updated.",
        });
      });
  }, [activeThread, isServerThread]);
  const archiveActiveThread = useCallback(async () => {
    ...
    if (settings.confirmThreadArchive) {
      const confirmed = await api.dialogs.confirm(
        `Archive thread \"${activeThread.title}\"?\nArchived threads can be restored later.`,
      );
      if (!confirmed) return;
    }
    try {
      await archiveThreadFromClient(api.orchestration, activeThread.id);
      await handleNewChat();
    } catch (error) {
      toastManager.add({
        type: "error",
        title: "Could not archive thread",
        description: error instanceof Error ? error.message : "The thread could not be archived.",
      });
    }
  }, [activeThread, handleNewChat, isServerThread, settings.confirmThreadArchive]);
  const copyActiveThreadTranscript = useCallback(async () => {
    if (!activeThread) return;
    ...
    try {
      ...
      toastManager.add({
        ...
        description: error instanceof Error ? error.message : "The transcript could not be copied.",
      });
    }
  }, [activeThread]);
  const moreMenuItems = activeThread ? (
    <>
      <MenuItem onClick={() => setRenameDialogOpen(true)}>
        <PencilIcon className="size-3.5 shrink-0" />
        <span className="min-w-0 flex-1">Rename</span>
      </MenuItem>
      <MenuItem disabled={!isServerThread} onClick={toggleActiveThreadPin}>
        <PinIcon className="size-3.5 shrink-0" />
        <span className="min-w-0 flex-1">{activeThread.isPinned ? "Unpin" : "Pin"}</span>
      </MenuItem>
      <MenuItem
        disabled={!isServerThread || Boolean(activeThread.parentThreadId)}
      ...
      <MenuSeparator />
      <MenuItem onClick={() => void copyActiveThreadTranscript()}>
        <CopyIcon className="size-3.5 shrink-0" />
        <span className="min-w-0 flex-1">Copy transcript</span>
      </MenuItem>
      <MenuItem onClick={() => copyThreadIdToClipboard(activeThread.id)}>
        <CopyIcon className="size-3.5 shrink-0" />
        <span className="min-w-0 flex-1">Copy thread ID</span>
      </MenuItem>
      {threadWorkspaceCwd ? (
        <MenuItem onClick={() => copyPathToClipboard(threadWorkspaceCwd)}>
          <CopyIcon className="size-3.5 shrink-0" />
      ...
      <MenuSeparator />
      <MenuItem
        disabled={!isServerThread || Boolean(activeThread.sidechatSourceThreadId)}
        onClick={() => void handleStandaloneSlashCommand("/side")}
      >
        <SidechatIcon className="size-3.5 shrink-0" />
        <span className="min-w-0 flex-1">New side chat</span>
      </MenuItem>
      <MenuItem
        disabled={!isServerThread}
        onClick={() => void handleStandaloneSlashCommand("/fork")}
      >
        <GitForkIcon className="size-3.5 shrink-0" />
        <span className="min-w-0 flex-1">Fork</span>
      </MenuItem>
      <MenuSeparator />
      <MenuItem
        disabled
        title="Scheduled tasks are unavailable in this build."
      >
        <ClockIcon className="size-3.5 shrink-0" />
        <span className="min-w-0 flex-1">Add scheduled task</span>
        <span className="ms-auto text-[10px] text-muted-foreground">Unavailable</span>
      </MenuItem>
      <MenuSeparator />
      <MenuItem disabled title="Additional Agent windows are unavailable in this build.">
        <ExternalLinkIcon className="size-3.5 shrink-0" />
        <span className="min-w-0 flex-1">Open in new window</span>
        <span className="ms-auto text-[10px] text-muted-foreground">Unavailable</span>
      </MenuItem>
    </>
  ) : null;
    ...
    <>
      <ComposerExtrasTrigger
        ...
        onToggle={() => {
          ...
          // The panel is keyboard-driven from the editor: keep the cedia where the
  ...
  const environmentPanelProps: Omit<EnvironmentPanelProps, "open" | "variant"> = {
    context: {
      ...
      onAddSource: (source) => {
        setComposerPromptValue(`${promptRef.current}${promptRef.current.trim() ? "\n\n" : ""}Source: ${source}`);
        scheduleComposerFocus();
      },
    },
    gitCwd: threadWorkspaceCwd,
                ...
                {activePendingApproval ? null : (
                  <ChatComposerFooter
                    ...
                    contextMeter={
                      ...
                      (selectedProvider === "omp" || composerFooterControlsPlan.showContextMeter) ? (
                        <ContextWindowMeter
          ...
          >
            {shouldRenderChatPaneContent && isCenteredEmptyLanding ? (
              ...
              >
                ...
                <div className={isIdeEmbeddedRuntime() ? "hidden" : "flex min-h-0 flex-1 items-center justify-center"}>
                  <div
                    className={cn(
                ...
                <div className={cn("w-full shrink-0 pb-3 sm:pb-4", isIdeEmbeddedRuntime() && "pt-3")}>
                  {composerSection}
                  {relocateComposerLeadingControls ? (

...
// OMP owns message IDs. Match an optimistic send only to a subsequent durable
// user row, consuming each row once so repeated prompts remain distinct.
export function acknowledgedOptimisticIds(server: readonly ChatMessage[], pending: readonly ChatMessage[], omp: boolean): Set<MessageId> {
  const ids = new Set(server.map(message => message.id));
  if (!omp) return ids;
  const used = new Set<MessageId>();
  for (const message of pending) {
    if (ids.has(message.id)) continue;
    const actual = server.find(row => row.role === "user" && !used.has(row.id) && row.text === message.text && Date.parse(row.createdAt) >= Date.parse(message.createdAt));
    if (actual) { used.add(actual.id); ids.add(message.id); }
  }
  return ids;
}
...
export function useChatTimelineMessages({
  ...
  const timelineMessages = useMemo(() => {
    ...
    if (optimisticUserMessages.length > 0) {
      const serverIds = acknowledgedOptimisticIds(serverMessagesWithPreviewHandoff, optimisticUserMessages, activeThread?.session?.provider === "omp");
  ...
  useEffect(() => {
    ...
    const serverIds = acknowledgedOptimisticIds(activeThread.messages, optimisticUserMessages, activeThread.session?.provider === "omp");

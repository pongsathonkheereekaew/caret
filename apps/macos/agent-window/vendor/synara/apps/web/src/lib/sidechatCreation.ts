...
export async function ensureSidechatSource(input: {
  api: NativeApi;
  project: Project;
  sourceThread: Thread;
  selectedModelSelection: ModelSelection;
  ...
  if (input.selectedModelSelection.provider !== "omp") return;
  const snapshot = await input.api.orchestration.getShellSnapshot();
  if (snapshot.threads.some((thread) => thread.id === input.sourceThread.id)) return;
  await input.api.orchestration.dispatchCommand({
    type: "thread.create",
    commandId: newCommandId(),
    ...
    modelSelection: input.selectedModelSelection,
    runtimeMode: input.runtimeMode ?? input.sourceThread.runtimeMode,
    interactionMode: "default",
    envMode: input.sourceThread.envMode ?? "local",
...
export async function createSidechatThread(input: {
  ...
  await ensureSidechatSource(input);
  ...
  await input.api.orchestration.dispatchCommand({
    ...
    importedMessages: input.selectedModelSelection.provider === "omp" ? [] : [...buildThreadHandoffImportedMessages(input.sourceThread)],
    createdAt,
  });

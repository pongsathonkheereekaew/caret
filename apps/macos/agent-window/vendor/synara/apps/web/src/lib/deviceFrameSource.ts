...
/**
 * Optional host-owned implementation. Cedia's Agent Window receives the same
 * binary envelope over authenticated Electron IPC, while browser/Synara
 * deployments continue to use the dedicated WebSocket below.
 */
...
export function createDeviceFrameSource(options: DeviceFrameSourceOptions): DeviceFrameSource {
  const nativeFactory = (globalThis as { __cediaNativeDeviceFrameSource?: NativeDeviceFrameFactory })
    .__cediaNativeDeviceFrameSource;
  const nativeSource = nativeFactory?.(options);

...
import { DragHandleIcon, PlusIcon } from "~/lib/icons";
...
export const ProviderModelMenuItems = function ProviderModelMenuItems(
  ...
  const [draggingProvider, setDraggingProvider] = useState<ProviderKind | null>(null);
  ...
  const providerOrder = props.providerOrder ?? settings.providerOrder;
  ...
  const moveProvider = (over: ProviderKind) => {
    if (!draggingProvider || draggingProvider === over) return;
    updateSettings({ providerOrder: moveProviderInOrder(providerOrder, draggingProvider, over) });
      ...
      {visibleAvailableProviderOptions.map((option) => {
        ...
        return (
          <MenuSub key={option.value}>
            <MenuSubTrigger
              draggable={visibleAvailableProviderOptions.length > 1}
              onDragStart={(event) => {
                setDraggingProvider(option.value);
                event.dataTransfer.effectAllowed = "move";
                event.dataTransfer.setData("text/provider", option.value);
              }}
              onDragOver={(event) => {
                if (!draggingProvider || draggingProvider === option.value) return;
                event.preventDefault();
                event.dataTransfer.dropEffect = "move";
              }}
              onDrop={(event) => {
                event.preventDefault();
                moveProvider(option.value);
              }}
              onDragEnd={() => setDraggingProvider(null)}
              className={cn(draggingProvider === option.value && "opacity-60")}
            >
              <DragHandleIcon
                aria-hidden="true"
                className={cn(
                  "size-3 shrink-0 cursor-grab text-muted-foreground/55",
                  visibleAvailableProviderOptions.length < 2 && "invisible",
                )}
              />
              <OptionIcon
                aria-hidden="true"

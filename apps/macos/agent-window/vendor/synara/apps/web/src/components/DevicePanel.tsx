...
export default function DevicePanel(props: {
  ...
  // Keep the optimistic pending device visible while booting, but do not open
  // the frame subscription until the attach RPC has committed the device to
  // this thread. Starting the stream from `pendingDevice` races that RPC and
  // the native host correctly rejects it as not attached yet.
  const streamDevice =
    threadState?.attachedDeviceUdid !== null && threadState?.attachedDeviceUdid === attachedDevice?.udid
      ? attachedDevice
      : null;
  const streamEnabled = shouldSubscribeToDeviceStream({
    runtimeMode,
    isVisible,
    attachedDevice: streamDevice,
  });
  ...
  const { status: videoStatus, dimensions } = useDeviceVideoStream({
    canvasRef,
    udid: streamEnabled && streamDevice ? streamDevice.udid : null,
  ...
  if (status.kind === "unsupported") {
    return (
      <p className="text-balance text-center text-[10px] text-white/70 leading-snug">
        ...
        WebCodecs video decoder Cedia uses.
      </p>
    );
  }
    ...
    <Dialog open={state !== null} onOpenChange={(open) => (open ? undefined : props.onDismiss())}>
      <DialogPopup>
        <DialogHeader>
          ...
          */}
          <DialogDescription>
            Cedia keeps at most {state?.limit ?? 0} simulators running at once, because each one

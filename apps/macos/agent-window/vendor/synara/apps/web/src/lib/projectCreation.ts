...
export async function createOrRecoverProjectFromPath(input: {
   ...
   * project's default model selection. Defaults to OMP when omitted, and legacy
   * Pi settings migrate to OMP because OMP owns the runtime model catalogue. */
  ...
  const seedProvider =
    input.defaultProvider === "pi" ? "omp" : (input.defaultProvider ?? "omp");
  ...
  try {
    await input.api.orchestration.dispatchCommand({
      ...
      defaultModelSelection: {
        ...
        model: getDefaultModel(seedProvider) ?? "cedia:unresolved",
      },

...
import { ArchivedThreadDeleteButton } from "./ArchivedThreadDeleteButton";
...
export function ArchivedSettingsPanel({ active }: { readonly active: boolean }) {
  ...
  const deleteArchivedThreadWithoutConfirm = useCallback(
  ...
  // Context-menu deletes have no inline affordance, so they keep the explicit
  // native confirmation dialog and then reuse the direct delete path.
  const deleteArchivedThread = useCallback(
    async (threadId: ThreadId, threadTitle: string) => {
      ...
      if (!confirmed) return;
      await deleteArchivedThreadWithoutConfirm(threadId);
    },
    [deleteArchivedThreadWithoutConfirm],
  );

  // Cedia addition (see upstream.json adaptations): archived threads pile up with no bulk way out,
  // so the panel carries one "Delete all" action next to the count. It reuses the same client helper
  // and the same children-before-parents order as the per-thread path.
  ...
  const deleteAllArchivedThreads = useCallback(async () => {
    const api = readNativeApi();
    if (!api || archivedThreadIds.length === 0) return;
    const confirmed = await api.dialogs.confirm(
      [
        `Permanently delete ${archivedThreadIds.length} archived ${pluralize(archivedThreadIds.length, "thread")}?`,
        "This will remove the threads and their conversation history forever.",
      ].join("\n"),
    );
    if (!confirmed) return;
    try {
      // Subagent threads are hidden from this list, so deleting a root removes its whole subtree;
      // children go first so a mid-flight failure cannot strand one without its parent entry.
      const threadIds = archivedThreadIds.flatMap((threadId) => [
      ...
      toastManager.add({
        ...
        title:
          archivedThreadIds.length === 1
            ? "Thread deleted"
            : `Deleted ${archivedThreadIds.length} archived threads`,
        description: "The archived conversation history has been permanently removed.",
      });
    } catch (error) {
      toastManager.add({
        type: "error",
        title: "Could not delete archived threads",
        description:
          error instanceof Error ? error.message : "Unable to delete the archived threads.",
      });
    }
  }, [archivedThreadIds, removeDeletedThreadFromClientState, threadShells]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <p className={SETTINGS_CARD_ROW_DESCRIPTION_CLASS_NAME}>
          {archivedThreadIds.length} archived {pluralize(archivedThreadIds.length, "thread")}
        </p>
        <Button size="xs" variant="destructive" onClick={() => void deleteAllArchivedThreads()}>
          Delete all
        </Button>
      </div>
            ...
            <SettingsListRow
              ...
              actions={
                <>
                  ...
                  <ArchivedThreadDeleteButton
                    threadTitle={thread.title}
                    onConfirm={() => deleteArchivedThreadWithoutConfirm(thread.id)}
                  />
                </>
              }
            />

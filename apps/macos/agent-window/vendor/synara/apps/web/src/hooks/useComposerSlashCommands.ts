...
export function useComposerSlashCommands(input: {
  ...
  const createSidechatFromSlashCommand = useCallback(
    (inputOptions?: { initialPrompt?: string; targetProvider?: ProviderKind }): Promise<true> => {
      ...
      if (
        ...
        !activeProject ||
        !activeThread ||
        (!isServerThread && !(selectedModelSelection.provider === "omp" && isLocalDraftThread)) ||
  ...
  useEffect(() => {
    if (!activeProject || !activeThread || (!isServerThread && !(selectedModelSelection.provider === "omp" && isLocalDraftThread)) || activeThread.sidechatSourceThreadId) {
      return;
    }
    return registerSidechatCreator(threadId, createSidechatFromSlashCommand);
  }, [activeProject, activeThread, createSidechatFromSlashCommand, isServerThread, isLocalDraftThread, selectedModelSelection.provider, threadId]);

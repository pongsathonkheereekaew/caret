...
const BUILT_IN_MODEL_SLUGS_BY_PROVIDER: Record<ProviderKind, ReadonlySet<string>> = {
  ...
  // OMP is runtime-discovered and has no baked-in model ids.
  omp: new Set<string>(),
};
...
export const AppSettingsSchema = Schema.Struct({
  ...
  defaultProvider: PersistedProviderKind.pipe(withDefaults(() => "omp" as const)),
...
export function getCustomModelsForProvider(
  ...
  return config ? (settings[config.settingsKey] ?? []) : [];
}
...
export function getDefaultCustomModelsForProvider(
  ...
  const config = PROVIDER_CUSTOM_MODEL_CONFIG[provider];
  return config ? (defaults[config.defaultSettingsKey] ?? []) : [];
}
...
export function getCustomModelsByProvider(
  ...
  return {
    ...
    // OMP's model list is owned by the native runtime and cannot be edited here.
    omp: [],
  };
}
...
export function getAppModelOptions(
  ...
  if (
    ...
    !(provider === "omp" && normalizedSelectedModel === "cedia:unresolved") &&
...
export function getGitTextGenerationModelOptions(
  ...
  const options = GIT_TEXT_GENERATION_PROVIDERS.flatMap((provider) => {
    ...
    const config = PROVIDER_CUSTOM_MODEL_CONFIG[provider];
    const customModels = config ? (settings[config.settingsKey] ?? []) : [];

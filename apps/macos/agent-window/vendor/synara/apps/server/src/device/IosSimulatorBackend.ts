...
} from "../../../../packages/shared/src/deviceHelperCache.ts";
...
export class IosSimulatorBackend implements DeviceBackend {
  ...
  constructor(options: IosSimulatorBackendOptions = {}) {
    ...
    this.xcodeBundleUsable =
      ...
      ((developerDir) =>
        // `xcodebuild` is shipped inside the Developer directory. `xcrun` is
        // a system tool at /usr/bin/xcrun, so checking for it under Xcode
        // incorrectly rejects a complete Xcode installation.
        access(path.join(developerDir, "usr", "bin", "xcodebuild")).then(
          () => true,
          () => false,
        ));
  }

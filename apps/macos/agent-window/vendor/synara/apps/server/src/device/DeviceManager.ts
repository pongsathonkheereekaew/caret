...
export class DeviceManager {
  ...
  subscribeFrames(udid: string, sink: DeviceFrameSink): () => void {
    ...
    // `attach()` starts the capture eagerly so the thread state can report
    // progress without waiting for a viewer. If that wins the race before the
    // pane subscribes, the helper has already emitted its one codec-config/
    // keyframe pair to nobody and `startStream()` would incorrectly treat the
    // active stream as complete. Restart once for a fresh subscriber so the
    // decoder always receives a new parameter set and IDR.
    const restartForViewer = this.activeStreamUdid === udid;
    void (restartForViewer ? this.requestKeyframe(udid) : this.startStream(udid)).catch(() => undefined);
    return () => {
      unsubscribe();

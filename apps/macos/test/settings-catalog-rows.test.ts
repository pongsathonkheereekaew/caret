...
import { bindSettingsCatalogRows, jsonPickerCommands, skillsFromSlashCommands, type SettingsBoundRow } from "../src/settings-catalog-rows.ts";
...
describe("bindSettingsCatalogRows", () => {
	...
	it("binds the skills collection OMP advertises as slash commands", () => {
		const rows = bindSettingsCatalogRows({
			section: "Skills/rules/hooks/commands",
			skills: [{ id: "tdd", label: "tdd" }],
			source: "x",
		});
		expect(byId(rows, "skill:tdd")).toMatchObject({
			section: "Skills/rules/hooks/commands",
			label: "tdd",
			value: "advertised",
			source: "x",
		});
		expect(rows.some((row) => row.id === "skill:none")).toBe(false);
	});
...
describe("skillsFromSlashCommands", () => {
	it("keeps only skill-sourced commands, deduped in first-seen order", () => {
		expect(skillsFromSlashCommands([
			{ name: "review" },
			{ name: "tdd", source: "skill" },
			{ name: "tdd", source: "skill" },
			{ name: "plan", source: "mcp_prompt" },
			{ name: "", source: "skill" },
		])).toEqual([{ id: "tdd", label: "tdd" }]);
	});
});

describe("jsonPickerCommands", () => {
	it("omits commands with dedicated Cedia UI while preserving order", () => {
		expect(jsonPickerCommands(["get_state", "prompt", "get_available_models", "abort"])).toEqual(["prompt", "abort"]);
		expect(jsonPickerCommands([])).toEqual([]);
	});
});

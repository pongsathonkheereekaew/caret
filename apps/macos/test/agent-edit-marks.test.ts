...
/*
 * The marks are what makes Caret's edit legible inside the editor, and the
...
describe("the label", () => {
	it("names the path, the regions, and the lines", () => {
		expect(agentEditLabel("src/app.ts", [{ start: pos(0, 0), end: pos(2, 0) }])).toBe("src/app.ts: 1 region, 3 lines changed by Caret");
		expect(agentEditLabel("src/app.ts", [{ start: pos(0, 0), end: pos(0, 1) }])).toBe("src/app.ts: 1 region, 1 line changed by Caret");
	});
});
...
describe("the revert rule", () => {
	...
	it("restores the exact pre-edit text while the buffer still holds Caret's version", () => {
...
describe("the painted range and hover", () => {
	...
	it("names the file and both actions in the hover", () => {
		...
		expect(decorationHover("src/app.ts")).toContain("Caret: Take Back the Caret Edit");
	});
});
...
describe("agent edit review decision", () => {
	...
	it("allows a review diff while the buffer still holds Caret's version", () => {
...
describe("agent edit lens specs", () => {
	...
	it("places both decisions on the first region Caret changed", () => {
		expect(agentEditLenses(pending)).toEqual([
			{ line: 4, title: "Keep the Caret Edit", command: "caret.keepAgentEdit" },
			{ line: 4, title: "Take Back the Caret Edit", command: "caret.revertAgentEdit" },
		]);
	});
});

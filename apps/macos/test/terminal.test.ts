...
const { CEDIA_TERMINAL_HISTORY_TRUNCATED_MARKER, OmpTerminalViews } = await import("../src/terminal.ts");
...
function openFrame(): Record<string, unknown> {
	return { type: "cedia_terminal_open", terminalId: "terminal-1", title: "OMP" };
}

describe("Cedia OMP terminal views", () => {
	it("preserves output when OMP ends before the view opens, including reopening ended history", () => {
		...
		views.ingest(event({ type: "cedia_terminal_output", terminalId: "terminal-1", sequence: 1, data: "final output" }, 2));
		views.ingest(event({ type: "cedia_terminal_close", terminalId: "terminal-1" }, 3));
	...
	it("reopens a closed view, replays retained output, and rejects stale input/close callbacks", async () => {
		...
		first.open();
		views.ingest(event({ type: "cedia_terminal_output", terminalId: "terminal-1", sequence: 1, data: "before\r\n" }, 2));
		// Same OMP sequence is a duplicate and must not be rendered twice.
		views.ingest(event({ type: "cedia_terminal_output", terminalId: "terminal-1", sequence: 1, data: "duplicate\r\n" }, 3));
		first.options.pty.handleInput?.("old-input");
		first.options.pty.close();
		views.ingest(event({ type: "cedia_terminal_output", terminalId: "terminal-1", sequence: 2, data: "while-closed\r\n" }, 4));
		await Promise.resolve();
		const inputCountBeforeReopen = sent.filter(item => item.command === "cedia_terminal_input").length;
		...
		expect(sent.filter(item => item.command === "cedia_terminal_resize")).toHaveLength(2);
		...
		await Promise.resolve();
		expect(sent.filter(item => item.command === "cedia_terminal_input")).toHaveLength(inputCountBeforeReopen + 1);
		...
		views.ingest(event({ type: "cedia_terminal_close", terminalId: "terminal-1" }, 5));
	...
	it("marks bounded history explicitly when the retained tail is truncated", () => {
		...
		views.ingest(event({ type: "cedia_terminal_output", terminalId: "terminal-1", sequence: 10, data: oversized }, 11));
		...
		expect(rendered).toContain(CEDIA_TERMINAL_HISTORY_TRUNCATED_MARKER);
		expect(Buffer.byteLength(rendered, "utf8")).toBeGreaterThan(Buffer.byteLength(CEDIA_TERMINAL_HISTORY_TRUNCATED_MARKER, "utf8"));
		views.dispose();
	});
	...
	it("forwards modified F3 and strips emulator queries from written output", async () => {
		...
		first.options.pty.onDidWrite(value => output.push(value));
		first.open();
		views.ingest(event({ type: "cedia_terminal_output", terminalId: "terminal-1", sequence: 1, data: "visible\x1b[6n\x1b]10;?;?\x07\x1b]4;0;?\x07more" }, 2));
		...
		await Promise.resolve();
		const inputs = sent.filter(item => item.command === "cedia_terminal_input");
		expect(inputs.map(item => item.payload.data)).toEqual(["\x1b[1;2R"]);
		views.ingest(event({ type: "cedia_terminal_output", terminalId: "terminal-1", sequence: 2, data: "\x1b[" }, 3));
		views.ingest(event({ type: "cedia_terminal_output", terminalId: "terminal-1", sequence: 3, data: "6nkept" }, 4));

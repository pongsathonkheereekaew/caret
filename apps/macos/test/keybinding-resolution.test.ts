...
describe("Cmd+K resolves to Cedia inline edit", () => {
	const cediaBinding = item(
		"cedia.inlineEdit",
		["cmd+k"],
	...
	it("fires the single-chord Cedia binding when a selection exists", () => {
		const kb = resolver([...builtInChords, cediaBinding]);
		...
		expect(result.kind === ResultKind.KbFound && result.commandId).toBe("cedia.inlineEdit");
	});
	...
	it("still waits for the chord when there is no selection", () => {
		...
		// Cedia's when-clause does not match, so the built-in chord prefix wins.
...
describe("line-number gutter menu is a live surface", () => {
	it("is rendered by the pinned workbench and normalizes the selection", async () => {
		...
		// VS Code selects the clicked line before showing the menu, so a Cedia

...
describe("selection actions", () => {
	it("ships Explain and Fix with stable command ids", () => {
		expect(SELECTION_ACTIONS.map(action => action.command)).toEqual(["cedia.explainSelection", "cedia.fixSelection"]);
		expect(selectionAction("explain").title).toBe("Cedia: Explain Selection");
		expect(selectionAction("fix").title).toBe("Cedia: Fix Selection");
	});

...
import { CediaHostClient, readHostDescriptor, validateHostDescriptor } from "../src/api.ts";

function response(status: number, body: unknown): Response {
	return new Response(body === undefined ? null : JSON.stringify(body), { status, headers: { "content-type": "application/json" } });
}

describe("Cedia host HTTP client", () => {
	...
	it("attaches bearer auth and uses the versioned API paths", async () => {
		...
		const client = new CediaHostClient({
	...
	it("reads a private descriptor and refuses an invalid one", async () => {
		const dir = join(tmpdir(), `cedia-host-${Date.now()}-${Math.random().toString(36).slice(2)}`);

...
describe("buildSearchHits", () => {
	it("labels New task without inventing a file index note on empty search", () => {
		...
		expect(SEARCH_INDEX_FAILED_NOTE).toBe("Last indexed: workspace file search failed. Cedia will not invent a file index.");
	...
	it("scopes tasks, files, actions, and settings without creating a task on no-match", () => {
		...
		expect(buildSearchHits({ query: "cedia-ui-no-match-137", sessions: [{ id: "s1", title: "Hello" }] })).toMatchObject({
			noMatch: true,
			hits: [],
		});
	});

...
export interface StubState {
	 ...
	 * them. Used to prove Cedia does not overwrite an explicit user choice. */
	...
	/** How many of the next registrations should fail because the vendor is not declared yet. */
	languageModelRegistrationFailures: number;
...
export const stubState: StubState = {
	...
	languageModelRegistrationFailures: 0,
	installedExtensions: [],
};
...
export function stubUri(value: string): any {
	...
	// Hierarchical authority for non-file URIs (e.g. `cedia.omp://session/s1`
...
/** `cedia-review:` style URIs keep their raw spelling so a parse/toString round
...
export function createVscodeStub(): unknown {
	...
	return {
		...
		ChatResponseTurn2: class {
			...
			constructor(response: readonly unknown[], result: unknown, participant: string) {
		...
		ChatQuestion: class {
			constructor(readonly id: string, readonly type: number, readonly title: string, readonly options?: unknown) {}
		},
		ChatQuestionType: { Text: 1, SingleSelect: 2, MultiSelect: 3 },
		...
		// registers is what the workbench uses to give a Cedia request a model.
		lm: {
			registerLanguageModelChatProvider: (vendor: string, provider: any) => {
				// The real workbench throws while it has not declared the vendor yet (Cedia's bridge does
				// that when the window restores), which is the race the extension has to survive.
				if (stubState.languageModelRegistrationFailures > 0) {
					stubState.languageModelRegistrationFailures -= 1;
		...
		// Cedia owns a token source for the refreshes it asks for itself, and the

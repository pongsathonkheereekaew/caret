...
describe("destinationOptions", () => {
	it("keeps Run on / Connect via / Workspace separate and does not invent Cloud", () => {
		const rows = destinationOptions({ hasGit: false, hostReachable: true, relayStatus: "unknown", projectName: "cedia" });
		...
		expect(rows.find((row) => row.id === "ws-current")?.label).toBe("cedia");
	});

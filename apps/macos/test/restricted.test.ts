...
test("restricted shell exposes only trust management and disposes before activating execution", async () => {
  ...
  const context = { subscriptions, extension: { packageJSON: { contributes: { commands:
    ["cedia.newTask", "cedia.openTerminal", "cedia.pairDevice", "cedia.openComposer"].map(command => ({ command })),
  } } } };
  ...
  for (const id of ["cedia.newTask", "cedia.openTerminal", "cedia.pairDevice"]) await commands.get(id)!();

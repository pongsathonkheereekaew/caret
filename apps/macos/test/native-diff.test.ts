...
describe("native diff document identity", () => {
	it("round-trips a workspace-relative path and ref", () => {
		...
		expect(id).toBe("caret-review:/HEAD/src/sync/client.ts");
	...
	it("rejects ids that are not review documents", () => {
		...
		expect(decodeReviewDocId("caret-review:/HEAD")).toBeUndefined();
		expect(decodeReviewDocId("caret-review:/")).toBeUndefined();
	});
});
...
describe("original-side read against a real repository", () => {
	function fixture(): string {
		const dir = mkdtempSync(join(tmpdir(), "caret-native-diff-"));
	...
	it("cannot be turned into an arbitrary command by a hostile ref", () => {
		const dir = fixture();
		try {
			const [subcommand, target] = reviewOriginalArgs("--output=/tmp/caret-pwned", "src/app.ts");
...
describe("Caret edit review diff", () => {
	...
	it("rejects a foreign or malformed id", () => {
		expect(decodeAgentEditDocId("caret-review:/HEAD/src/greet.ts")).toBeUndefined();
	...
	it("names both sides of the Caret edit in the title", () => {
		expect(agentEditDiffTitle("src/greet.ts")).toBe("src/greet.ts (before Caret ↔ after Caret)");
	});
});

...
describe("cediaWindowTitle", () => {
	it("uses task — project — Cedia in Agents when a task title exists", () => {
		expect(cediaWindowTitle({
			mode: "agents",
			taskTitle: "Polish header",
			projectName: "cedia",
		})).toBe("Polish header — cedia — Cedia");
	});

	it("drops the project segment when Agents has a task but no project", () => {
		expect(cediaWindowTitle({ mode: "agents", taskTitle: "Polish header" })).toBe("Polish header — Cedia");
	});

	it("uses New task — Cedia for an Agents empty draft and never invents a session id", () => {
		expect(cediaWindowTitle({ mode: "agents" })).toBe("New task — Cedia");
		expect(cediaWindowTitle({ mode: "agents", taskTitle: "   " })).toBe("New task — Cedia");
		expect(cediaWindowTitle({ mode: "agents", projectName: "cedia" })).toBe("New task — Cedia");
		expect(cediaWindowTitle({ mode: "agents", fileName: "webview.ts" })).toBe("New task — Cedia");
		expect(cediaWindowTitle({ mode: "agents" })).not.toMatch(/sess(?:ion)?[-_]/i);
		expect(cediaWindowTitle({ mode: "agents", projectName: "cedia" })).not.toContain("sess-");
	});

	it("uses file — project — Cedia in IDE when a file is open", () => {
		expect(cediaWindowTitle({
			mode: "ide",
			fileName: "task-chrome.ts",
			projectName: "cedia",
			taskTitle: "ignored",
		})).toBe("task-chrome.ts — cedia — Cedia");
	});

	it("uses file — Cedia in IDE when a file is open without a project", () => {
		expect(cediaWindowTitle({ mode: "ide", fileName: "task-chrome.ts" })).toBe("task-chrome.ts — Cedia");
	});

	it("uses project — Cedia or Cedia in IDE when no file is open", () => {
		expect(cediaWindowTitle({ mode: "ide", projectName: "cedia", taskTitle: "ignored" })).toBe("cedia — Cedia");
		expect(cediaWindowTitle({ mode: "ide", fileName: "  " })).toBe(CEDIA_BRAND);
		expect(cediaWindowTitle({ mode: "ide" })).toBe("Cedia");
	});
});
...
describe("taskHeaderMeta", () => {
	it("shows an advertised branch and never invents one", () => {
		...
		expect(meta.worktree).toBe("/tmp/cedia-feat");
		expect(meta.path).toBe("/Users/pond/cedia");
		expect(meta.cwd).toBe("/tmp/cedia-feat");
		expect(meta.summary).toBe("cedia · feat/s02 · /tmp/cedia-feat · /Users/pond/cedia");
	...
	it("shows an advertised parent session and never invents one", () => {
		const withParent = taskHeaderMeta({ projectName: "cedia", parentSession: "sess-parent" });
		expect(withParent.parentSession).toBe("sess-parent");
		expect(withParent.summary).toContain("parent sess-parent");
		expect(taskHeaderMeta({ projectName: "cedia" }).parentSession).toBeUndefined();
	});
});
...
describe("moreMenuActions", () => {
	...
	it("disables split when the active pane cannot fit another min size", () => {
		...
		expect(tight.find((action) => action.id === "split_right")).toMatchObject({
			enabled: false,
			reason: "Need more space. Maximize area or Open another window. Cedia will not squeeze this pane.",
		});

...
describe("Cedia task webview", () => {
	it("uses a nonce CSP and safe DOM rendering primitives", () => {
		const html = createTaskWebviewHtml({ cspSource: "vscode-resource://cedia" }, "nonce123");
		expect(html).toContain("script-src 'nonce-nonce123'");
		expect(html).toContain("style-src vscode-resource://cedia 'nonce-nonce123'");
		...
		expect(html).toContain("--cedia-sidebar: 180px");
		expect(html).toContain("min-width: 160px");
		expect(html).toContain("--cedia-font-xs: 11px");
		expect(html).toContain("--cedia-font-sm: 12px");
		expect(html).toContain("--cedia-font-base: 13px");
		expect(html).toContain("--cedia-font-lg: 14px");
		expect(html).toContain("--cedia-lh-xs: 14px");
		expect(html).toContain("--cedia-lh-sm: 16px");
		expect(html).toContain("--cedia-lh-base: 18px");
		expect(html).toContain("--cedia-lh-lg: 22px");
		...
		expect(html).toContain("--cedia-row: 30px");
		expect(html).toContain("--cedia-sidebar-icon: 13px");
		...
		expect(html).toContain("--cedia-transcript-column: 608px");
		expect(html).toContain("--cedia-composer-column: 608px");
		// D20 parity locks: values that define the Cursor-matched shell. These
		// must not drift back to raw literals or cediaake-only defaults.
		expect(html).toContain("--cedia-control-radius: 6px");
		// The reference's prompt-input radii are per state (compact radius-full,
		// island radius-3xl, expanded radius-4xl); Cedia's multi-line composer is
		// the expanded shape, so 18px - not the 10/12px Cedia shipped.
		expect(html).toContain("--cedia-composer-radius: 18px");
		expect(html).toContain("--cedia-composer-editor-min: 36px");
		expect(html).toContain("--cedia-composer-editor-max: 200px");
		...
		expect(html).toContain("font-size: var(--cedia-font-lg); line-height: var(--cedia-lh-lg)");
		...
		expect(html).toContain("--cedia-top-cluster: 124px");
		expect(html).toContain("padding-right: calc(var(--cedia-space-12) + var(--cedia-top-cluster))");
		expect(html).toContain(".send-round { width: auto; height: var(--cedia-height-lg); min-width: var(--cedia-height-lg); padding: 0 var(--cedia-space-10); border-radius: var(--cedia-radius-full)");
		expect(html).not.toContain("border-radius: 50%; background: var(--cedia-text)");
		...
		expect(html).toContain("--cedia-motion-instant: 50ms");
		expect(html).toContain("--cedia-motion-drawer-in: 200ms");
		expect(html).toContain("--cedia-motion-drawer-out: 150ms");
		expect(html).toContain("cubic-bezier(0.215, 0.61, 0.355, 1)");
		expect(html).toContain("--cedia-motion-drawer-in");
		...
		expect(TASK_WEBVIEW_SCRIPT).not.toContain("workbench.view.scm");
		expect(TASK_WEBVIEW_SCRIPT).toContain("presentation-notify");
		expect(TASK_WEBVIEW_SCRIPT).toContain("presentation-status");
		expect(TASK_WEBVIEW_SCRIPT).toContain("presentation-widget");
		expect(TASK_WEBVIEW_SCRIPT).toContain("presentation-editor-text");
		expect(TASK_WEBVIEW_SCRIPT).toContain("notifyType");
	});
	...
	it("gives the dock's model picker a provider icon and names why a row cannot be used", () => {
		// The dock owns this control (the Agents-window pill is the workbench's), so it is the surface
		// where a provider can be shown the way the reference does: the catalogue is OMP's, and the same
		// model id can come from more than one provider (measured live 2026-09-18), which a flat list
		// cannot say. Rows are grouped under their provider with a monogram chip, and a row OMP reports
		// as unusable carries that reason instead of only being greyed out.
		// A bundled vendor glyph is built node by node from path data carried in the snapshot; the
		// monogram is only the fallback, and both the trigger and each row draw through one helper.
		expect(TASK_WEBVIEW_SCRIPT).toContain("const providerGlyphsFor = (provider)");
		expect(TASK_WEBVIEW_SCRIPT).toContain("document.createElementNS('http://www.w3.org/2000/svg', 'svg')");
		expect(TASK_WEBVIEW_SCRIPT).toContain("svg.setAttribute('viewBox', glyph.viewBox)");
		expect(TASK_WEBVIEW_SCRIPT).toContain("function chipFor(provider)");
		// Every row wears its provider's mark, the same one the group header and the trigger draw: the icon
		// answers "which provider/api does this run through", which is the choice this picker makes.
		expect(TASK_WEBVIEW_SCRIPT).toContain("row.appendChild(chipFor(key));");
		expect(TASK_WEBVIEW_SCRIPT).not.toContain("vendorMarkFor");
		expect(TASK_WEBVIEW_SCRIPT).toContain("paintTriggerMark(");
		expect(TASK_WEBVIEW_SCRIPT).toContain("function providerInitials(provider)");
		expect(TASK_WEBVIEW_SCRIPT).toContain("function providerHue(provider)");
		expect(TASK_WEBVIEW_SCRIPT).toContain("function chipFor(provider)");
		expect(TASK_WEBVIEW_SCRIPT).toContain("chip.className = 'provider-chip'");
		// Two columns, like the reference's picker: providers down the left with their mark and count,
		// the chosen provider's models down the right, search across both.
		expect(TASK_WEBVIEW_SCRIPT).toContain("className = 'provider-row'");
		expect(TASK_WEBVIEW_SCRIPT).toContain("name.className = 'provider-row-name'");
		expect(TASK_WEBVIEW_SCRIPT).toContain("chev.className = 'provider-row-chev'");
		expect(TASK_WEBVIEW_SCRIPT).toContain("row.dataset.provider = key");
		expect(TASK_WEBVIEW_SCRIPT).toContain("activeProvider = provider.dataset.provider");
		expect(TASK_WEBVIEW_SCRIPT).toContain("from.className = 'model-row-provider'");
		expect(TASK_WEBVIEW_SCRIPT).toContain("row.className = 'model-row'");
		expect(TASK_WEBVIEW_SCRIPT).toContain("model.reason === 'string' && model.reason ? model.reason : 'not available'");
		expect(TASK_WEBVIEW_SCRIPT).toContain("post({ type: 'select_model', modelId: row.dataset.modelId })");
		expect(TASK_WEBVIEW_SCRIPT).toContain("modelSearch.addEventListener('input', () => renderModels())");
		// The markup keeps the trigger's id (`model-select`) so the composer's snapshot wiring still
		// disables the control through `controls.modelEnabled`, and adds the panel it opens.
		const html = createTaskWebviewHtml({ cspSource: "vscode-resource://cedia" }, "nonce123");
		expect(html).toContain('id="model-chip"');
		expect(html).toContain('id="model-panel"');
		expect(html).toContain('id="model-search"');
		expect(html).toContain('aria-haspopup="listbox"');
		expect(html).toContain('placeholder="Search models or providers"');
		expect(html).toContain('id="model-providers"');
	});
});

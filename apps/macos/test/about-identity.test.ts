...
const ABOUT_IDS = ["cedia", "extension", "codeoss", "omp", "protocol", "sourceHash", "updates"] as const;
...
describe("aboutIdentity", () => {
	it("keeps missing OMP unavailable and does not invent a marketed version", () => {
		const rows = aboutIdentity({ cediaVersion: "0.1.0" });
		...
		expect(byId(rows, "cedia")).toEqual({
			id: "cedia",
			label: "Cedia build",
			value: "0.1.0",
			claim: "advertised",
		...
		expect(rows.some((row) => /notariz|latest|1\.0\.0/i.test(row.value) && row.id !== "cedia")).toBe(false);
	});

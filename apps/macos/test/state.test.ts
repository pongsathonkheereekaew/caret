...
describe("Cedia webview message boundary", () => {
	it("accepts known actions and rejects arbitrary host actions", () => {
		...
		expect(parseWebviewMessage({ type: "open_recent", path: "/Users/pond/cedia" })).toEqual({ type: "open_recent", path: "/Users/pond/cedia" });
	...
	it("projects every OMP presentation method with its own validated fields", () => {
		const apply = (request: Record<string, unknown>) => applyFrame(createInitialTaskState(), {
			type: "cedia_ui",
			event: { kind: "presentation", request },
		} as Json).presentations;
		expect(apply({ id: "n1", method: "notify", message: "Heads up", notifyType: "warning" })).toEqual([{ id: "n1", method: "notify", message: "Heads up", notifyType: "warning" }]);
		expect(apply({ id: "s1", method: "setStatus", statusKey: "build", statusText: "Compiling" })).toEqual([{ id: "s1", method: "setStatus", statusKey: "build", statusText: "Compiling" }]);
		expect(apply({ id: "w1", method: "setWidget", widgetKey: "todo", widgetLines: ["one", "two"], widgetPlacement: "aboveEditor" })).toEqual([{ id: "w1", method: "setWidget", widgetKey: "todo", widgetLines: ["one", "two"], widgetPlacement: "aboveEditor" }]);
		expect(apply({ id: "t1", method: "setTitle", title: "Build the thing" })).toEqual([{ id: "t1", method: "setTitle", title: "Build the thing" }]);
		expect(apply({ id: "e1", method: "set_editor_text", text: "" })).toEqual([{ id: "e1", method: "set_editor_text", text: "" }]);
		expect(apply({ id: "s2", method: "setStatus" })).toEqual([]);
		expect(apply({ id: "n2", method: "notify", message: "Heads up", notifyType: "loud" })).toEqual([]);
		expect(apply({ id: "x1", method: "setTheme", title: "nope" })).toEqual([]);
	});
	it("projects OMP slash commands from the commands list and live updates", () => {
		...
		expect(normalizeSlashCommands({ commands: [{ name: "tdd", description: "Test-first", source: "skill" }] })).toEqual([{ name: "tdd", description: "Test-first", source: "skill" }]);
		expect(normalizeSlashCommands({ commands: [{ name: "review", source: 7 }, { name: "plan", source: "  " }] })).toEqual([{ name: "review" }, { name: "plan" }]);

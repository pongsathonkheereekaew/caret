...
export interface CaretWindowTitleInput {
...
export const CARET_BRAND = "Caret";
...
export const NEED_MORE_SPACE_REASON = "Need more space. Maximize area or Open another window. Caret will not squeeze this pane.";
...
export function caretWindowTitle(input: CaretWindowTitleInput): string {
	const projectName = clean(input.projectName);
	if (input.mode === "ide") {
		const fileName = clean(input.fileName);
		if (fileName && projectName) return joinTitle(fileName, projectName, CARET_BRAND);
		if (fileName) return joinTitle(fileName, CARET_BRAND);
		if (projectName) return joinTitle(projectName, CARET_BRAND);
		return CARET_BRAND;
	}
	const taskTitle = clean(input.taskTitle);
	if (!taskTitle) return joinTitle(NEW_TASK_LABEL, CARET_BRAND);
	if (projectName) return joinTitle(taskTitle, projectName, CARET_BRAND);
	return joinTitle(taskTitle, CARET_BRAND);
}

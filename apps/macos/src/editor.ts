...
export function parseEditorRequest(value: unknown): EditorRequest | undefined {
	if (!isRecord(value) || value.protocolVersion !== CEDIA_EDITOR_PROTOCOL_VERSION) return undefined;
	...
	switch (value.kind) {
		case "read":
			return nonEmptyString(value.path, MAX_PATH_LENGTH)
				? { protocolVersion: CEDIA_EDITOR_PROTOCOL_VERSION, requestId, kind: "read", path: value.path }
				: undefined;
		case "inventory":
			return value.includeClean === undefined || typeof value.includeClean === "boolean"
				? {
					protocolVersion: CEDIA_EDITOR_PROTOCOL_VERSION,
					requestId,
					kind: "inventory",
 ...
 */
export class CediaEditorService {
	...
	private bind(document: TextDocument, target: { readonly path: string; readonly root: WorkspaceRoot }): DocumentBinding {
		...
		const handle: EditorDocumentHandle = { id: newId("cedia-doc"), uri };
	...
	private async applyDocument(api: VscodeApi, request: EditorApplyInput): Promise<EditorApplyResult> {
		...
		return this.withLock(lockKey, async () => {
			...
			return {
				kind: "apply",
				requestId: request.requestId ?? newId("cedia-edit"),
	...
	async create(path: string, content?: string): Promise<EditorCreateResult> {
		return this.createDocument(await this.api(), path, content, newId("cedia-create"));
	}

	async delete(input: EditorGuardInput & { requestId?: string }): Promise<EditorDeleteResult> {
		return this.deleteDocument(await this.api(), { ...input, requestId: input.requestId ?? newId("cedia-delete") });
	}

	async move(input: EditorGuardInput & { destination: string; requestId?: string }): Promise<EditorMoveResult> {
		return this.moveDocument(await this.api(), { ...input, requestId: input.requestId ?? newId("cedia-move") });
	}

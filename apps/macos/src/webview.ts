...
:root {
	...
	--cedia-font-xs: 11px; --cedia-font-sm: 12px; --cedia-font-base: 13px; --cedia-font-lg: 14px;
	--cedia-lh-xs: 14px; --cedia-lh-sm: 16px; --cedia-lh-base: 18px; --cedia-lh-lg: 22px;
	--cedia-height-xs: 20px; --cedia-height-sm: 24px; --cedia-height-base: 28px; --cedia-height-lg: 32px;
	 ...
	 * box is what a user sees (hover height, list rhythm) and Cedia matches the
	...
	--cedia-space-4: 4px; --cedia-space-6: 6px; --cedia-space-8: 8px; --cedia-space-10: 10px; --cedia-space-12: 12px; --cedia-space-16: 16px; --cedia-space-20: 20px; --cedia-space-24: 24px; --cedia-space-28: 28px; --cedia-space-32: 32px; --cedia-space-40: 40px; --cedia-space-44: 44px; --cedia-space-48: 48px;
	--cedia-radius-xs: 2px; --cedia-radius-sm: 4px; --cedia-radius-md: 6px; --cedia-radius: 8px; --cedia-radius-xl: 12px; --cedia-radius-2xl: 14px; --cedia-radius-3xl: 16px; --cedia-radius-4xl: 18px; --cedia-radius-full: 9999px;
	 ...
	 * composer with a toolbar, which is what Cedia's is - is radius-4xl (18).
	 * Cedia used 10px then 12px, neither of which is a composer radius in the
	 * reference's scale. */
	--cedia-control-radius: 6px; --cedia-composer-radius: 18px;
	/* Reference --prompt-input-editor-min-height = spacing-9 = 36px and
	 * --prompt-input-editor-max-height = 200px. */
	--cedia-composer-editor-min: 36px; --cedia-composer-editor-max: 200px;
	...
	--cedia-transcript-column: 608px; --cedia-composer-column: 608px; --cedia-composer-column-outer: calc(var(--cedia-composer-column) + 2 * var(--cedia-gutter));
	--cedia-sidebar: 180px; --cedia-panel-w: 360px; --cedia-panel-h: 40vh; --cedia-gutter: 24px;
...
body { margin: 0; min-width: 0; background: var(--cedia-bg); color: var(--cedia-text); font: var(--cedia-font-base)/var(--cedia-lh-base) var(--vscode-font-family, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif); }
a { color: var(--cedia-link); text-decoration: underline; }
button, input, textarea, select { font: inherit; }
button { border: 1px solid transparent; border-radius: var(--cedia-control-radius); background: transparent; color: inherit; cursor: pointer; min-height: var(--cedia-height-base); min-width: var(--cedia-height-base); padding: 0 var(--cedia-space-8); }
button:hover:not(:disabled) { background: var(--cedia-hover-bg); transition: background var(--cedia-motion-feedback) var(--cedia-motion-curve); }
button:focus-visible, input:focus-visible, textarea:focus-visible, select:focus-visible { outline: 2px solid var(--cedia-focus); outline-offset: 2px; }
button.primary { background: var(--cedia-accent); color: var(--cedia-accent-text); font-weight: 500; }
button.ghost { color: var(--cedia-muted); }
button.danger { color: var(--cedia-danger); }
button:disabled { cursor: default; opacity: .55; }
input, textarea, select { border: 1px solid var(--cedia-control-border); border-radius: var(--cedia-control-radius); background: var(--cedia-input); color: var(--cedia-text); padding: 7px 9px; }
input::placeholder, textarea::placeholder { color: var(--cedia-muted); }
.shell { display: grid; grid-template-columns: var(--cedia-sidebar) 2px minmax(360px, 1fr); height: 100vh; overflow: hidden; }
.shell.work-open { grid-template-columns: var(--cedia-sidebar) 2px minmax(360px, 1fr) 2px var(--cedia-panel-w); }
.shell.sidebar-closed { grid-template-columns: minmax(360px, 1fr); }
.shell.sidebar-closed.work-open { grid-template-columns: minmax(360px, 1fr) 2px var(--cedia-panel-w); }
...
.sash { width: 2px; background: var(--cedia-border); cursor: col-resize; }
.sash:focus-visible { outline: 2px solid var(--cedia-focus); outline-offset: 0; }
.shell.work-bottom.work-open { grid-template-columns: var(--cedia-sidebar) 2px minmax(360px, 1fr); grid-template-rows: minmax(240px, 1fr) 2px var(--cedia-panel-h); }
...
.shell.work-bottom.work-open .resources { grid-column: 1 / -1; width: auto; max-width: none; min-width: 0; border-left: 0; border-top: 1px solid var(--cedia-border); }
.sr-only { position: absolute; width: 1px; height: 1px; padding: 0; margin: -1px; overflow: hidden; clip: rect(0, 0, 0, 0); white-space: nowrap; border: 0; }
.queue-item .meta { color: var(--cedia-muted); font-size: 12px; line-height: 18px; }
...
.sidebar, .resources { display: flex; min-width: 0; flex-direction: column; background: var(--cedia-panel); }
.sidebar { width: var(--cedia-sidebar); min-width: 160px; max-width: 360px; border-right: 1px solid var(--cedia-border); }
.resources { width: var(--cedia-panel-w); min-width: 280px; max-width: 640px; border-left: 1px solid var(--cedia-border); }
.sidebar-head, .resource-head, .topbar { display: flex; align-items: center; gap: var(--cedia-space-8); min-width: 0; min-height: 46px; padding: 0 var(--cedia-space-12); border-bottom: 1px solid var(--cedia-border); }
...
@media (min-width: 620px) { .topbar { padding-right: calc(var(--cedia-space-12) + var(--cedia-top-cluster)); } }
...
.brand-mark { width: 16px; height: 16px; flex: 0 0 auto; color: var(--cedia-text); }
.sidebar-head .spacer, .topbar .spacer, .resource-head .spacer { flex: 1; }
.sidebar-search { padding: var(--cedia-space-8) var(--cedia-space-12); }
.attention { min-width: 22px; min-height: 22px; border-radius: var(--cedia-radius-full); background: var(--cedia-warning); color: var(--cedia-bg); font-size: 12px; line-height: 22px; text-align: center; padding: 0 6px; }
.sidebar-search input { width: 100%; }
.section-label { display: flex; align-items: center; padding: var(--cedia-space-16) var(--cedia-space-12) var(--cedia-space-8); color: var(--cedia-text); font-size: 14px; font-weight: 600; line-height: 22px; }
.list { overflow: auto; padding: 0 var(--cedia-space-4) var(--cedia-space-16); }
.project, .session { display: flex; align-items: center; gap: 4px; width: 100%; min-height: var(--cedia-row); height: var(--cedia-row); margin: 0; padding: 0 var(--cedia-space-4); border-radius: var(--cedia-radius-sm); text-align: left; font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); }
.project:hover, .session:hover { background: var(--cedia-hover-bg); }
.project.selected, .session.selected { background: var(--cedia-selected-bg); color: var(--cedia-selected-fg); }
.project .name, .session .name { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.project .meta, .session .meta { display: block; color: var(--cedia-muted); font-size: 12px; line-height: 18px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.session .dot { width: 6px; height: 6px; border-radius: 50%; background: var(--cedia-muted); flex: 0 0 auto; }
.session.running .dot { background: var(--cedia-success); }
.session.unknown .dot, .session.recovery_required .dot { background: var(--cedia-warning); }
.session .status-label { color: var(--cedia-muted); font-size: var(--cedia-font-xs); line-height: var(--cedia-lh-xs); }
.sidebar-foot { margin-top: auto; padding: var(--cedia-space-12); border-top: 1px solid var(--cedia-border); display: grid; gap: 8px; }
.mode-switch { display: inline-flex; border: 1px solid var(--cedia-control-border); border-radius: var(--cedia-control-radius); }
.mode-switch button { min-width: 72px; min-height: 32px; border-radius: 0; font-weight: 500; }
.mode-switch button:first-child { border-radius: var(--cedia-control-radius) 0 0 var(--cedia-control-radius); }
.mode-switch button:last-child { border-radius: 0 var(--cedia-control-radius) var(--cedia-control-radius) 0; }
.mode-switch button[aria-pressed="true"] { background: var(--cedia-selected-bg); color: var(--cedia-selected-fg); }
.mode-switch button, .ide-btn { transition: none; }
.main { display: flex; min-width: 0; flex-direction: column; background: var(--cedia-bg); }
...
.task-project { color: var(--cedia-muted); font-size: 12px; line-height: 18px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.connection { display: inline-flex; align-items: center; gap: var(--cedia-space-6); min-height: var(--cedia-height-sm); padding: 0 var(--cedia-space-6); border: 1px solid var(--cedia-control-border); border-radius: var(--cedia-radius-sm); color: var(--cedia-muted); font-size: var(--cedia-font-xs); line-height: var(--cedia-lh-xs); }
.connection::before { content: ""; width: 8px; height: 8px; border-radius: 50%; background: currentColor; }
.connection.running { color: var(--cedia-success); }
.connection.unknown, .connection.connecting { color: var(--cedia-warning); }
.connection.offline { color: var(--cedia-danger); }
.run { display: inline-flex; align-items: center; min-height: var(--cedia-height-sm); padding: 0 var(--cedia-space-6); color: var(--cedia-muted); font-size: var(--cedia-font-xs); line-height: var(--cedia-lh-xs); }
.run.running, .run.waiting { color: var(--cedia-warning); }
.transcript-wrap { position: relative; flex: 1; min-height: 0; overflow: auto; }
.transcript { max-width: var(--cedia-transcript-column); margin: 0 auto; padding: var(--cedia-space-16) var(--cedia-gutter) var(--cedia-space-24); }
.message-body { max-width: 72ch; white-space: pre-wrap; overflow-wrap: anywhere; user-select: text; font-size: var(--cedia-font-lg); line-height: var(--cedia-lh-lg); font-weight: 400; }
.empty { display: grid; min-height: 240px; place-items: center; color: var(--cedia-muted); text-align: center; padding: var(--cedia-space-24); }
.empty strong { display: block; margin-bottom: 8px; color: var(--cedia-text); font-size: 18px; font-weight: 600; line-height: 26px; }
...
/* The docked composer carries --cedia-gutter inline padding (line below), which
...
.main.is-empty .composer { padding: 0; max-width: var(--cedia-composer-column); width: min(var(--cedia-composer-column), calc(100% - 48px)); margin: 0 auto; transform: none; border-top: 0; }
.main.is-empty .composer-box { border-radius: var(--cedia-composer-radius); box-shadow: none; padding: 12px; }
...
.message-head { display: flex; align-items: baseline; gap: 8px; margin-bottom: 4px; color: var(--cedia-muted); font-size: 12px; line-height: 18px; }
.message-head strong { color: var(--cedia-text); font-size: 12px; }
.message.user { padding: 12px; border: 1px solid var(--cedia-border); border-radius: var(--cedia-radius); background: var(--cedia-panel); }
.tool-card { margin: 8px 0 12px; border: 1px solid var(--cedia-border); border-radius: var(--cedia-radius); overflow: hidden; }
...
.tool-state { color: var(--cedia-muted); font-size: 12px; }
.tool-state.running { color: var(--cedia-warning); }
.tool-state.completed { color: var(--cedia-success); }
.tool-state.failed { color: var(--cedia-danger); }
.tool-details { display: block; max-height: 0; opacity: 0; overflow: hidden; padding: 0 12px; border-top: 0; transition: max-height var(--cedia-motion-surface-in) var(--cedia-motion-curve), opacity var(--cedia-motion-surface-in) linear; }
.tool-card.expanded .tool-details { max-height: 240px; opacity: 1; padding: 0 12px 12px; border-top: 1px solid var(--cedia-border); overflow: auto; }
.tool-card.full-log.expanded .tool-details, .tool-card.full-log .tool-details pre { max-height: none; }
.tool-details pre { max-height: 240px; margin: 8px 0; padding: 8px; overflow: auto; border: 1px solid var(--cedia-border); border-radius: 4px; background: var(--cedia-input); font: 13px/20px var(--vscode-editor-font-family, ui-monospace, monospace); white-space: pre-wrap; overflow-wrap: anywhere; }
.tool-actions { display: flex; justify-content: flex-end; gap: 8px; }
.event-row { margin: 8px 0; padding: 8px; border-left: 2px solid var(--cedia-border); color: var(--cedia-muted); font-size: 12px; }
.composer { padding: var(--cedia-space-12) var(--cedia-gutter) var(--cedia-space-16); border-top: 1px solid var(--cedia-border); }
.composer-target { max-width: var(--cedia-composer-column); margin: 0 auto var(--cedia-space-8); color: var(--cedia-muted); font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); }
.composer-box { max-width: var(--cedia-composer-column); margin: 0 auto; border: 1px solid var(--cedia-control-border); border-radius: var(--cedia-composer-radius); background: var(--cedia-composer-surface); box-shadow: none; }
.composer textarea { display: block; width: 100%; min-height: var(--cedia-composer-editor-min); max-height: var(--cedia-composer-editor-max); resize: vertical; border: 0; border-radius: var(--cedia-composer-radius) var(--cedia-composer-radius) 0 0; background: transparent; padding: var(--cedia-space-8) var(--cedia-space-12); font-size: var(--cedia-font-lg); line-height: var(--cedia-lh-lg); }
.composer textarea:focus { outline: none; }
.chip-row { display: flex; flex-wrap: wrap; gap: 8px; padding: 0 12px 8px; }
.chip { display: inline-flex; align-items: center; gap: var(--cedia-space-6); min-height: var(--cedia-height-sm); padding: 0 var(--cedia-space-6); border: 1px solid var(--cedia-control-border); border-radius: var(--cedia-radius-sm); color: var(--cedia-muted); font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); }
.composer-controls { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; padding: 8px; border-top: 1px solid var(--cedia-border); }
...
.shortcut { color: var(--cedia-muted); font-size: 12px; line-height: 18px; max-width: 28ch; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 0 1 auto; }
...
.model-select { max-width: 200px; min-height: var(--cedia-height-sm); border: 0; background: transparent; color: var(--cedia-muted); }
.jump-latest { display: none; position: sticky; bottom: 8px; margin: 0 auto 8px; }
.jump-latest.visible { display: inline-flex; }
.queue-list { max-width: var(--cedia-composer-column); margin: 0 auto var(--cedia-space-8); }
.queue-item { padding: 8px; border: 1px solid var(--cedia-border); border-radius: 8px; color: var(--cedia-muted); font-size: 12px; }
.resource-tabs { display: flex; flex-wrap: nowrap; gap: 4px; overflow: auto; padding: 8px; border-bottom: 1px solid var(--cedia-border); }
.resource-tabs button { min-height: 32px; }
.resource-tabs button[aria-selected="true"] { background: var(--cedia-selected-bg); color: var(--cedia-selected-fg); }
.resources .resource-body { overflow: auto; padding: var(--cedia-space-16); }
.resource-card { margin-bottom: 16px; padding: 12px; border: 1px solid var(--cedia-border); border-radius: var(--cedia-radius); }
.resource-card h3 { margin: 0 0 8px; font-size: 14px; line-height: 22px; font-weight: 600; }
.resource-card p { margin: 0 0 12px; color: var(--cedia-muted); font-size: 12px; line-height: 18px; }
.resource-action { display: flex; align-items: center; width: 100%; gap: 8px; margin: 4px 0; padding: 8px 12px; border: 1px solid var(--cedia-control-border); text-align: left; }
...
.login-row .meta { color: var(--cedia-muted); font-size: 12px; line-height: 18px; }
.presentations { display: grid; gap: 8px; margin-top: 8px; }
.presentation-notify { padding: 8px 12px; border: 1px solid var(--cedia-border); border-left-width: 3px; border-radius: var(--cedia-radius); background: var(--cedia-panel-raised); }
.presentation-notify p { margin: 0; color: var(--cedia-muted); font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); white-space: pre-wrap; }
.presentation-notify[data-notify-type="warning"] { border-left-color: var(--cedia-warning); }
.presentation-notify[data-notify-type="error"] { border-left-color: var(--cedia-danger); }
.presentation-notify[data-notify-type="warning"] p { color: var(--cedia-warning); }
.presentation-notify[data-notify-type="error"] p { color: var(--cedia-danger); }
.presentation-status { margin: 0; color: var(--cedia-muted); font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); }
.presentation-widget { padding: 8px 12px; border: 1px solid var(--cedia-border); border-radius: var(--cedia-radius); background: var(--cedia-panel-raised); }
.presentation-widget h4 { margin: 0 0 4px; font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); }
.presentation-widget p { margin: 0; color: var(--cedia-muted); font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); white-space: pre-wrap; }
.presentation-title { margin: 0; color: var(--cedia-muted); font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); }
.presentation-editor-text pre { margin: 0; padding: 8px; overflow: auto; border: 1px solid var(--cedia-border); border-radius: 4px; background: var(--cedia-input); font: 13px/20px var(--vscode-editor-font-family, ui-monospace, monospace); white-space: pre-wrap; overflow-wrap: anywhere; }
.status-line { min-height: var(--cedia-lh-sm); max-width: var(--cedia-composer-column); margin: var(--cedia-space-8) auto 0; color: var(--cedia-muted); font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); }
.ui-requests { position: absolute; inset-inline: var(--cedia-gutter); bottom: 16px; z-index: 2; display: grid; gap: 8px; max-width: 560px; margin: 0 auto; }
.ui-card { padding: 16px; border: 1px solid var(--cedia-warning); border-radius: 12px; background: var(--cedia-panel-raised); box-shadow: var(--cedia-elevate-2); }
.ui-card h3 { margin: 0 0 8px; font-size: 14px; line-height: 22px; }
.ui-card p { margin: 0 0 12px; color: var(--cedia-muted); white-space: pre-wrap; }
.ui-card textarea, .ui-card input, .ui-card select { width: 100%; margin: 0 0 12px; }
.ui-card.readonly { border-color: var(--cedia-border); opacity: .92; }
.ui-meta { margin: 0 0 12px; color: var(--cedia-muted); font-size: 12px; line-height: 18px; white-space: pre-wrap; }
.ui-actions { display: flex; justify-content: flex-end; gap: 8px; }
.pref-row { display: flex; flex-wrap: wrap; gap: 8px; margin: 12px 0; }
.pref-row button[aria-pressed="true"] { background: var(--cedia-selected-bg); color: var(--cedia-selected-fg); }
.term-log { margin: 0 0 12px; padding: 8px; max-height: 280px; overflow: auto; border: 1px solid var(--cedia-border); border-radius: 6px; background: var(--cedia-input); font: 13px/20px var(--vscode-editor-font-family, ui-monospace, monospace); white-space: pre-wrap; }
.term-meta { color: var(--cedia-muted); font-size: 12px; }
.reconnect-banner { display: none; padding: 8px 12px; background: color-mix(in srgb, var(--cedia-warning) 15%, transparent); color: var(--cedia-warning); font-size: 12px; line-height: 18px; text-align: center; }
.reconnect-banner.visible { display: block; }
.search-empty { padding: 8px 12px 16px; color: var(--cedia-muted); font-size: 12px; line-height: 18px; }
@media (max-width: 1199px) { :root { --cedia-sidebar: 180px; --cedia-gutter: 16px; } }
@media (max-width: 899px) { .shell, .shell.work-open { grid-template-columns: minmax(0, 1fr); } .sidebar { display: none; } .shell.sidebar-drawer .sidebar { display: flex; position: fixed; inset: 0 auto 0 0; width: min(320px, 86vw); max-width: 360px; z-index: 6; transform: translateX(-24px); opacity: 0; pointer-events: none; transition: transform var(--cedia-motion-drawer-in) var(--cedia-motion-curve), opacity var(--cedia-motion-drawer-in) linear; } .shell.sidebar-drawer.sidebar-open .sidebar { transform: none; opacity: 1; pointer-events: auto; } .shell.work-open .resources { position: absolute; inset: 0; width: auto; max-width: none; min-width: 0; border: 0; z-index: 3; } :root { --cedia-gutter: 16px; } }
@media (max-width: 619px) { :root { --cedia-gutter: 12px; } .topbar { flex-wrap: wrap; min-height: 46px; } .shortcut { display: none; } }
...
.sidebar { background: var(--cedia-panel); }
.main { background: var(--cedia-bg); }
.nav-row, .search-row { display: flex; align-items: center; gap: var(--cedia-space-4); width: 100%; margin: 0; padding: 0 var(--cedia-space-4); border-radius: var(--cedia-radius-sm); text-align: left; color: var(--cedia-text); font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); min-height: var(--cedia-row); height: var(--cedia-row); }
.nav-row .nav-icon, .search-row .nav-icon { width: var(--cedia-sidebar-icon); height: var(--cedia-sidebar-icon); flex: 0 0 auto; color: var(--cedia-muted); }
.nav-row.current .nav-icon, .nav-row:hover:not(:disabled) .nav-icon { color: var(--cedia-text); }
.nav-row .nav-label { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.nav-row.current, .nav-row:hover:not(:disabled) { background: color-mix(in srgb, var(--cedia-text) 8%, transparent); }
.search-row { position: relative; color: var(--cedia-muted); gap: 8px; }
.search-row > span { position: absolute; width: 1px; height: 1px; overflow: hidden; clip: rect(0, 0, 0, 0); }
.search-row input { position: static; flex: 1; min-width: 0; opacity: 1; border: 0; background: transparent; color: var(--cedia-text); padding: 0; }
.search-row:focus-within { background: color-mix(in srgb, var(--cedia-text) 8%, transparent); color: var(--cedia-text); }
.session-row { display: flex; align-items: center; gap: 2px; width: 100%; }
.session-row .session-open { flex: 1; min-width: 0; }
.ui-error { margin: 0 0 8px; color: var(--cedia-danger); font-size: 12px; line-height: 18px; }
.worktree-receipt { margin: 8px 0 0; color: var(--cedia-muted); font-size: 12px; line-height: 18px; }
.session-menu { flex: 0 0 auto; min-width: var(--cedia-row); min-height: var(--cedia-row); padding: 0; color: var(--cedia-muted); }
.code-block { margin: 8px 0; border: 1px solid var(--cedia-border); border-radius: 6px; overflow: hidden; background: var(--cedia-input); }
.code-head { display: flex; align-items: center; gap: 8px; padding: 4px 8px; border-bottom: 1px solid var(--cedia-border); color: var(--cedia-muted); font-size: 12px; }
...
.md-table th, .md-table td { border: 1px solid var(--cedia-border); padding: 6px 8px; text-align: left; }
.md-table th { color: var(--cedia-muted); font-weight: 600; }
.space-note-actions { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px; }
.nav-heading { display: flex; align-items: center; justify-content: space-between; gap: var(--cedia-space-6); min-height: 32px; margin: var(--cedia-space-10) 0 0; padding: 0 var(--cedia-space-6); color: var(--cedia-muted); font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); font-weight: 500; }
...
.nav-heading .nav-heading-label:hover:not(:disabled) { background: transparent; color: var(--cedia-text); }
.nav-heading .nav-icon { width: 16px; height: 16px; }
.icon-quiet { min-width: 32px; min-height: 32px; padding: 0; color: var(--cedia-muted); }
.session, .project { border-radius: var(--cedia-radius-sm); min-height: var(--cedia-row); height: var(--cedia-row); gap: 4px; padding: 0 var(--cedia-space-4); }
.session .meta, .project .meta { margin-left: auto; color: var(--cedia-muted); font-size: var(--cedia-font-sm); }
.nav-foot { margin-top: auto; padding: var(--cedia-space-4) var(--cedia-space-4) var(--cedia-space-12); border-top: 0; }
.ide-btn { min-width: 52px; border: 0; color: var(--cedia-muted); font-weight: 500; }
...
.welcome-kicker { font-size: 12px; letter-spacing: 0.08em; color: var(--cedia-muted); }
...
.pane-chip { min-height: 32px; border-radius: var(--cedia-control-radius); border: 1px solid var(--cedia-border); background: var(--cedia-panel); color: var(--cedia-text); padding: 0 10px; }
.pane-chip.active { border-color: var(--cedia-accent); }
...
.split-sash { flex: 0 0 2px; background: var(--cedia-border); cursor: col-resize; z-index: 3; }
.split-host.split-column .split-sash { cursor: row-resize; }
.pane-draft { flex: 1; min-height: 72px; width: 100%; resize: none; background: var(--cedia-input); color: var(--cedia-text); border: 1px solid var(--cedia-control-border); border-radius: var(--cedia-control-radius); padding: 8px; }
.pane-head { display: flex; align-items: center; gap: 8px; min-height: 40px; padding: 0 12px; border-bottom: 1px solid var(--cedia-border); }
.pane-transcript { flex: 1; min-height: 0; overflow: auto; }
.pane-composer { display: flex; gap: 8px; align-items: flex-end; padding: 8px; border-top: 1px solid var(--cedia-border); }
.work-tab-badge { margin-left: 6px; min-width: 8px; min-height: 8px; border-radius: 4px; background: var(--cedia-warning); }
.work-tab-badge.error { background: var(--cedia-danger); }
...
.main.is-empty .composer { padding: 0; max-width: var(--cedia-composer-column); width: min(var(--cedia-composer-column), calc(100% - 48px)); margin: 0 auto; transform: none; border-top: 0; }
.main.is-empty .composer-box { border: 1px solid var(--cedia-control-border); border-radius: var(--cedia-composer-radius); background: var(--cedia-composer-surface); box-shadow: none; padding: var(--cedia-space-12); }
 ...
 * Cedia shipped rows=3, which made the empty card 46px taller than the
 * reference's 106px. */
.main.is-empty textarea { border: 0; background: transparent; height: var(--cedia-composer-editor-min); min-height: var(--cedia-composer-editor-min); max-height: var(--cedia-composer-editor-min); resize: none; padding: 0; }
.main.is-empty .composer-bar { display: flex; align-items: center; gap: var(--cedia-space-8); padding: 0; }
...
.plus { width: 32px; height: 32px; min-width: 32px; padding: 0; border-radius: 50%; border: 1px solid var(--cedia-control-border); }
.plus-menu { display: grid; margin-top: 8px; border: 1px solid var(--cedia-border); border-radius: 8px; background: var(--cedia-panel); box-shadow: var(--cedia-elevate-1); }
.plus-menu[hidden], .more-menu[hidden] { display: none; }
.menu-empty { padding: 8px 12px; color: var(--cedia-muted); font-size: 12px; line-height: 18px; }
.more-menu { position: absolute; right: 12px; top: 46px; z-index: 4; display: grid; min-width: 220px; border: 1px solid var(--cedia-border); border-radius: 8px; background: var(--cedia-panel); box-shadow: var(--cedia-elevate-1); }
...
.mode-pills { margin: var(--cedia-space-8) auto var(--cedia-space-16); justify-content: flex-start; }
.pill { border: 1px solid var(--cedia-control-border); border-radius: var(--cedia-radius-full); min-height: 32px; padding: 0 var(--cedia-space-10); background: transparent; color: var(--cedia-text); font-size: var(--cedia-font-base); line-height: var(--cedia-lh-base); }
.pill[disabled] { color: var(--cedia-muted); }
/* The reference puts the branch/This Mac selector row INSIDE the prompt-input
 * card, above the editor, not in a separate band above it. Cedia had it in
 * .home-stage, which both added a band the reference does not have and left
 * the card 46px shorter than the reference's. */
.target-pills { align-items: center; justify-content: flex-start; gap: 2px; margin: 0 0 var(--cedia-space-8); padding: 0; }
.target-pills .pill { display: inline-flex; align-items: center; gap: var(--cedia-space-4); min-height: var(--cedia-height-xs); padding: 0 var(--cedia-space-4); border: 0; border-radius: var(--cedia-radius-sm); background: transparent; color: var(--cedia-muted); font-size: var(--cedia-font-base); line-height: var(--cedia-lh-base); }
.target-pills .pill:hover:not(:disabled) { background: var(--cedia-hover-bg); color: var(--cedia-text); }
.target-pills .pill::after { content: ""; width: 5px; height: 5px; margin-left: 2px; border-right: 1.3px solid currentColor; border-bottom: 1.3px solid currentColor; border-radius: var(--cedia-radius-xs); transform: rotate(45deg) translate(-1px, -1px); }
...
.send-round { width: auto; height: var(--cedia-height-lg); min-width: var(--cedia-height-lg); padding: 0 var(--cedia-space-10); border-radius: var(--cedia-radius-full); background: var(--cedia-text); color: var(--cedia-bg); font-weight: 600; white-space: nowrap; transition: transform var(--cedia-motion-surface-in) var(--cedia-motion-curve); }
.send-round:active { transform: scale(.96); }
.ideas { display: grid; gap: 0; max-width: var(--cedia-composer-column); margin: 0 auto; }
.ideas button { display: flex; align-items: center; gap: var(--cedia-space-8); width: 100%; min-height: 26px; height: 26px; text-align: left; padding: 0; color: var(--cedia-text); font-size: var(--cedia-font-base); line-height: var(--cedia-lh-base); border-radius: 0; border-bottom: 1px solid var(--cedia-border); }
.ideas .nav-icon { width: 16px; height: 16px; flex: 0 0 auto; color: var(--cedia-muted); }
.ideas .idea-title { color: var(--cedia-text); flex: 0 0 auto; }
.ideas .idea-copy { color: var(--cedia-muted); min-width: 0; }
...
.model-select { display: inline-flex; align-items: center; gap: 6px; max-width: 220px; min-width: 0; cursor: pointer; }
.model-select:disabled { cursor: default; }
/* A provider is the first thing that tells two models apart, and the dock has no brand assets: a
   monogram chip carries the provider where a logo would, and works for every provider OMP lists. */
.provider-chip { --provider-hue: 210; display: inline-flex; align-items: center; justify-content: center; flex: 0 0 auto; width: 18px; height: 18px; border-radius: 4px; font-size: 9px; font-weight: 700; color: hsl(var(--provider-hue) 70% 88%); background: hsl(var(--provider-hue) 45% 26%); border: 1px solid hsl(var(--provider-hue) 40% 36%); }
.provider-mark { display: inline-flex; align-items: center; justify-content: center; flex: 0 0 auto; width: 18px; height: 18px; }
.provider-glyph { width: 13px; height: 13px; flex: 0 0 auto; color: var(--cedia-muted); }
.model-row:hover:not(:disabled) .provider-glyph { color: inherit; }
.model-label { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.model-chev { color: var(--cedia-muted); font-size: 9px; }
.model-chev::before { content: "\25be"; }
.model-panel { position: absolute; bottom: calc(100% + 6px); left: 0; z-index: 5; display: flex; flex-direction: column; gap: 4px; width: 420px; max-height: 340px; padding: 6px; border: 1px solid var(--cedia-border); border-radius: 8px; background: var(--cedia-bg); box-shadow: 0 8px 24px rgba(0, 0, 0, 0.35); }
.model-columns { display: flex; gap: 4px; min-height: 0; }
.provider-column { flex: 0 0 46%; max-height: 268px; overflow: auto; display: flex; flex-direction: column; gap: 2px; }
.model-column { flex: 1 1 auto; max-height: 268px; overflow: auto; display: flex; flex-direction: column; gap: 2px; border-left: 1px solid var(--cedia-border); padding-left: 4px; }
.provider-row { display: flex; align-items: center; gap: 6px; width: 100%; border: 0; border-radius: 4px; background: transparent; color: inherit; text-align: left; padding: 5px 6px; cursor: pointer; }
.provider-row:hover { background: var(--cedia-hover-bg); }
.provider-row.active { background: var(--cedia-hover-bg); }
.provider-row-name { flex: 1 1 auto; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.provider-row-count { flex: 0 0 auto; color: var(--cedia-muted); font-size: 10px; }
.provider-row-chev { flex: 0 0 auto; color: var(--cedia-muted); font-size: 9px; }
.provider-row-chev::before { content: "\203a"; }
.model-row-chev { flex: 0 0 auto; color: var(--cedia-muted); font-size: 9px; }
.model-row-chev::before { content: "\203a"; }
.model-row-provider { flex: 0 0 auto; color: var(--cedia-muted); font-size: 10px; }
.model-panel[hidden] { display: none; }
.model-panel-search { width: 100%; min-height: var(--cedia-height-sm); border: 0; border-bottom: 1px solid var(--cedia-border); background: transparent; color: inherit; padding: 4px 2px; }
.provider-head { display: flex; align-items: center; gap: 6px; padding: 6px 4px 2px; color: var(--cedia-muted); font-size: 11px; }
.provider-name { flex: 1 1 auto; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.provider-count { flex: 0 0 auto; color: var(--cedia-muted); font-size: 10px; }
.model-row { display: flex; align-items: baseline; gap: 8px; width: 100%; border: 0; border-radius: 6px; background: transparent; color: inherit; text-align: left; padding: 5px 6px; cursor: pointer; }
.model-row:hover:not(:disabled) { background: var(--cedia-hover-bg); }
.model-row:disabled { color: var(--cedia-muted); cursor: default; }
.model-row-name { flex: 1 1 auto; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.model-row-reason, .model-row-current { flex: 0 0 auto; color: var(--cedia-muted); font-size: 10px; }
.settings-route { position: fixed; inset: 0; z-index: 6; display: flex; flex-direction: column; background: var(--cedia-bg); opacity: 0; visibility: hidden; pointer-events: none; transform: translateX(24px); transition: opacity var(--cedia-motion-drawer-out) linear, transform var(--cedia-motion-drawer-out) var(--cedia-motion-curve); }
.settings-route.open { opacity: 1; visibility: visible; pointer-events: auto; transform: none; transition-duration: var(--cedia-motion-drawer-in), var(--cedia-motion-drawer-in); }
...
.plus-menu { transition: opacity var(--cedia-motion-surface-in) linear, transform var(--cedia-motion-surface-in) var(--cedia-motion-curve); transform: translateY(4px); opacity: 0; }
.plus-menu:not([hidden]) { opacity: 1; transform: none; }
.route-error { position: fixed; inset: 0; z-index: 7; display: none; place-items: center; background: color-mix(in srgb, var(--cedia-bg) 88%, transparent); }
.route-error.open { display: grid; }
.route-error-card { max-width: 42em; margin: 24px; padding: 24px; border: 1px solid var(--cedia-border); border-radius: 12px; background: var(--cedia-panel-raised); box-shadow: var(--cedia-elevate-2); }
.route-error-card h2 { margin: 0 0 8px; font-size: 18px; line-height: 26px; }
.route-error-card p { margin: 0 0 16px; color: var(--cedia-muted); }
.history-banner { margin: 0 0 12px; padding: 8px 12px; border: 1px solid var(--cedia-warning); border-radius: 8px; color: var(--cedia-warning); font-size: 12px; line-height: 18px; }
.preview-frame { min-height: 160px; margin: 12px 0; padding: 16px; border: 1px dashed var(--cedia-border); border-radius: 8px; color: var(--cedia-muted); }
.settings-source, .settings-apply-error { color: var(--cedia-muted); font-size: 12px; line-height: 18px; }
.settings-apply-error { color: var(--cedia-danger); }
.settings-apply-row { display: flex; flex-wrap: wrap; gap: 8px; margin: 16px 0 0; }
.settings-top { display: flex; align-items: center; gap: 8px; min-height: 46px; padding: 0 12px; border-bottom: 1px solid var(--cedia-border); }
.settings-body { display: grid; grid-template-columns: 180px minmax(0, 1fr); min-height: 0; flex: 1; }
.settings-nav { overflow: auto; padding: 8px; border-right: 1px solid var(--cedia-border); }
.settings-nav button { display: block; width: 100%; text-align: left; margin: 2px 0; }
.settings-nav button[aria-current="true"] { background: var(--cedia-selected-bg); color: var(--cedia-selected-fg); }
.settings-page { overflow: auto; padding: 16px 24px 32px; }
.settings-page h2 { margin: 0 0 8px; font-size: 18px; line-height: 26px; }
.settings-page p { color: var(--cedia-muted); max-width: 72ch; }
.settings-search { min-width: 160px; margin-left: auto; }
.plan-strip { max-width: var(--cedia-composer-column); margin: 0 auto var(--cedia-space-8); padding: var(--cedia-space-8) var(--cedia-space-12); border: 1px solid var(--cedia-border); border-radius: var(--cedia-radius); }
.plan-strip h3 { margin: 0 0 6px; font-size: 13px; }
.plan-step { display: flex; gap: 8px; font-size: 12px; color: var(--cedia-muted); }
.review-row { display: flex; align-items: center; gap: 8px; width: 100%; text-align: left; margin: 2px 0; }
.review-row .mark { min-width: 18px; color: var(--cedia-muted); font: 13px/20px var(--vscode-editor-font-family, ui-monospace, monospace); }
...
.scope-chips button { min-height: var(--cedia-height-xs); padding: 0 var(--cedia-space-6); color: var(--cedia-muted); font-size: var(--cedia-font-xs); line-height: var(--cedia-lh-xs); }
.scope-chips button[aria-pressed="true"] { background: var(--cedia-selected-bg); color: var(--cedia-selected-fg); }
.sidebar-filters { display: flex; flex-wrap: wrap; gap: var(--cedia-space-4); padding: 2px var(--cedia-space-6) var(--cedia-space-8); }
.sidebar-filters[hidden] { display: none; }
.sidebar-filters button { min-height: var(--cedia-height-xs); padding: 0 var(--cedia-space-6); border: 1px solid var(--cedia-control-border); border-radius: var(--cedia-radius-full); color: var(--cedia-muted); font-size: var(--cedia-font-xs); line-height: var(--cedia-lh-xs); }
#filter-empty.empty { min-height: 0; padding: var(--cedia-space-4) var(--cedia-space-10) var(--cedia-space-8); place-items: start; justify-items: start; text-align: left; font-size: var(--cedia-font-sm); line-height: var(--cedia-lh-sm); }
...
.hunk { margin: 8px 0; border: 1px solid var(--cedia-border); border-radius: 6px; overflow: auto; }
.hunk-head { padding: 6px 8px; color: var(--cedia-muted); font: 13px/20px var(--vscode-editor-font-family, ui-monospace, monospace); }
...
.mention-menu { display: grid; margin: 0 12px 8px; border: 1px solid var(--cedia-border); border-radius: 8px; background: var(--cedia-panel); }
.mention-menu[hidden] { display: none; }
.message-body a { color: var(--cedia-link); }
.find-bar { display: flex; flex-wrap: wrap; align-items: center; gap: var(--cedia-space-8); max-width: var(--cedia-composer-column-outer); margin: 0 auto; padding: var(--cedia-space-8) var(--cedia-gutter); }
.find-bar[hidden] { display: none; }
.find-bar input { flex: 1; min-width: 160px; }
mark.find-hit { background: var(--cedia-warning); color: var(--cedia-bg); }
mark.find-hit.current { outline: 2px solid var(--cedia-focus); outline-offset: 1px; }
.destination-menu { display: grid; margin-top: 8px; border: 1px solid var(--cedia-border); border-radius: 8px; background: var(--cedia-panel); padding: 8px; gap: 4px; }
.destination-menu[hidden] { display: none; }
#space-note { display: none; margin: 0 var(--cedia-gutter, 20px) 8px; color: var(--cedia-muted); font-size: 12px; line-height: 18px; }
.shell.need-more-space #space-note { display: block; }
.shell.high-contrast { --cedia-elevate-1: none; --cedia-elevate-2: none; }
...
.thinking-select { max-width: 140px; min-height: var(--cedia-height-sm); border: 0; background: transparent; color: var(--cedia-muted); }
.destination-group { color: var(--cedia-muted); font-size: 12px; line-height: 18px; margin-top: 8px; }
...
(() => {
  ...
  let previewZoom = 1;
  // Provider marks arrive with the snapshot (extension.ts -> providerGlyphMap) so this script
  // stays framework-free: a vendor glyph where the catalogue bundles one, the monogram where not.
  const providerGlyphsFor = (provider) => { const key = String(provider || '').trim().toLowerCase(); const map = state.providerGlyphs || {}; return map[key] || null; };
  ...
  function renderStatus() {
    ...
    document.title = (state.session && state.session.title ? state.session.title : 'New task') + ' — ' + (state.project && state.project.name ? state.project.name : 'Cedia') + ' — Cedia';
    applyEmptyHome();
    renderWelcome();
    renderPaneStrip();
    renderSplit();
  ...
  function openSessionRowMenu(sessionId, pinned, archived) {
    ...
    [['Open in split', 'open_in_split'], ['Rename', 'rename'], [pinned ? 'Unpin' : 'Pin', pinned ? 'unpin' : 'pin'], [archived ? 'Restore' : 'Archive', archived ? 'restore' : 'archive'], ['Delete', 'delete']].forEach((item) => {
  ...
  function renderSessions() {
    ...
    if (bar) {
      ...
      markBtn.title = 'Clears unread on the filtered results in this window. Cedia will not mark every project read.';
  ...
  function messageNode(entry) {
    ...
    const head = element('div', 'message-head'); head.appendChild(element('strong', '', entry.role === 'user' ? 'You' : entry.role === 'system' ? 'Cedia' : 'Cedia')); head.appendChild(element('span', '', entry.status === 'streaming' ? 'typing…' : ''));
  ...
  function buildUiCard(token, request) {
    ...
    const heading = element('h3', '', request.title || 'Cedia needs your input'); heading.dataset.uiHeading = 'true'; card.appendChild(heading);
    ...
    if (request.method === 'schemaform') card.appendChild(element('p', '', 'Unsupported interaction. Cedia will not render untrusted form HTML. Update or Cancel.'));
  ...
  function updateUiCard(record, item) {
    const request = item.request || {};
    const heading = record.card.querySelector('[data-ui-heading]'); if (heading) text(heading, request.title || 'Cedia needs your input');
  ...
  function providerLabel(model) { return typeof model.provider === 'string' && model.provider ? model.provider : 'Other'; }
  // No brand assets ship with the dock, so a provider is drawn as its initials with a hue derived
  // from its id: stable per provider, distinguishable at a glance, and honest about what it is.
  function providerInitials(provider) { const parts = String(provider || 'Other').split(/[-_. ]+/).filter(Boolean); const letters = parts.length > 1 ? (parts[0][0] || '') + (parts[1][0] || '') : (parts[0] || '').slice(0, 2); return letters.toUpperCase() || '?'; }
  function providerHue(provider) { const text = String(provider || 'other'); let hash = 0; for (let i = 0; i < text.length; i += 1) { hash = (hash * 31 + text.charCodeAt(i)) % 360; } return hash; }
  function glyphNode(glyph) { const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg'); svg.setAttribute('viewBox', glyph.viewBox); svg.setAttribute('class', 'provider-glyph'); svg.setAttribute('aria-hidden', 'true'); glyph.paths.forEach((d) => { const path = document.createElementNS('http://www.w3.org/2000/svg', 'path'); path.setAttribute('d', d); path.setAttribute('fill', 'currentColor'); svg.appendChild(path); }); return svg; }
  // One mark per provider, shared by the trigger and every row: the vendor's own glyph when the
  // catalogue bundles one, otherwise the initials monogram, so a provider always has an identity.
  function chipFor(provider) { const mark = document.createElement('span'); mark.className = 'provider-mark'; mark.title = provider; const glyph = providerGlyphsFor(provider); if (glyph) { mark.appendChild(glyphNode(glyph)); return mark; } const chip = document.createElement('span'); chip.className = 'provider-chip'; chip.textContent = providerInitials(provider); chip.style.setProperty('--provider-hue', String(providerHue(provider))); mark.appendChild(chip); return mark; }
  // The panel is the reference's shape: providers down the left with their mark, the chosen
  // provider's models down the right, and a search that reaches across both. A picker that asks
  // "which provider/api runs this" has to make the provider the first choice, not a header buried in
  // five hundred rows.
  let activeProvider = null;
  function renderModels() {
    const trigger = byId('model-select'); const chipHost = byId('model-chip'); const label = byId('model-label');
    const providers = byId('model-providers'); const options = byId('model-options'); const search = byId('model-search');
    if (!trigger || !chipHost || !label || !providers || !options) return;
    const catalogue = state.models || [];
    const selected = state.selectedModel || '';
    const selectedModel = catalogue.filter((model) => model.id === selected)[0];
    const shown = selectedModel || catalogue[0];
    chipHost.title = shown ? providerLabel(shown) : '';
    paintTriggerMark(shown ? providerLabel(shown) : 'Other');
    label.textContent = selectedModel ? (selectedModel.label || selectedModel.id) : 'Model';
    providers.textContent = ''; options.textContent = '';
    if (!catalogue.length) { trigger.disabled = true; trigger.title = 'Choose a model. Use Add \u2192 Model to refresh. Cedia does not pick a billed fallback.'; return; }
    trigger.title = selectedModel ? (selectedModel.label || selectedModel.id) + ' \u00b7 ' + providerLabel(selectedModel) : 'Choose a model';
    const query = search && typeof search.value === 'string' ? search.value.trim().toLowerCase() : '';
    const groups = []; const byProvider = {};
    catalogue.forEach((model) => { const key = providerLabel(model); (byProvider[key] = byProvider[key] || []).push(model); if (groups.indexOf(key) === -1) groups.push(key); });
    const matches = (model, key) => !query || (model.label || model.id).toLowerCase().indexOf(query) !== -1 || key.toLowerCase().indexOf(query) !== -1;
    // The provider column: every provider the catalogue uses, its mark, how many models it carries.
    groups.forEach((key) => {
      const providerRows = byProvider[key].filter((model) => matches(model, key));
      if (query && !providerRows.length) return;
      const row = document.createElement('button');
      row.type = 'button'; row.className = 'provider-row' + (key === activeProvider ? ' active' : '');
      row.dataset.provider = key;
      row.appendChild(chipFor(key));
      const name = document.createElement('span'); name.className = 'provider-row-name'; name.textContent = key; row.appendChild(name);
      const count = document.createElement('span'); count.className = 'provider-row-count'; count.textContent = String(query ? providerRows.length : byProvider[key].length); row.appendChild(count);
      const chev = document.createElement('span'); chev.className = 'provider-row-chev'; chev.setAttribute('aria-hidden', 'true'); row.appendChild(chev);
      row.title = 'Show the models ' + key + ' runs';
      providers.appendChild(row);
    });
    // The model column: with no query, the active provider's models; with a query, every match, each
    // row naming its provider because one column no longer stands for one provider.
    const active = activeProvider && byProvider[activeProvider] ? activeProvider : (selectedModel ? providerLabel(selectedModel) : groups[0]);
    const columns = query ? groups.map((key) => ({ key, models: byProvider[key].filter((model) => matches(model, key)) })) : [{ key: active, models: (byProvider[active] || []).filter((model) => matches(model, active)) }];
    columns.forEach((column) => {
      if (!column.models.length) return;
      column.models.forEach((model) => {
        const unusable = model.available === false;
        const why = typeof model.reason === 'string' && model.reason ? model.reason : 'not available';
        const row = document.createElement('button');
        row.type = 'button'; row.className = 'model-row'; row.dataset.modelId = model.id;
        row.disabled = unusable; row.setAttribute('role', 'option'); row.setAttribute('aria-selected', model.id === selected ? 'true' : 'false');
        const rowLabel = document.createElement('span'); rowLabel.className = 'model-row-name'; rowLabel.textContent = model.label || model.id; row.appendChild(rowLabel);
        if (query) { const from = document.createElement('span'); from.className = 'model-row-provider'; from.textContent = column.key; row.appendChild(from); }
        if (unusable) { const reason = document.createElement('span'); reason.className = 'model-row-reason'; reason.textContent = why; row.appendChild(reason); row.title = why; }
        else if (model.id === selected) { const current = document.createElement('span'); current.className = 'model-row-current'; current.textContent = 'Current'; row.appendChild(current); row.title = (model.label || model.id) + ' \u00b7 ' + column.key; }
        else { row.title = (model.label || model.id) + ' \u00b7 ' + column.key; }
        const chev = document.createElement('span'); chev.className = 'model-row-chev'; chev.setAttribute('aria-hidden', 'true'); row.appendChild(chev);
        options.appendChild(row);
      });
    });
    if (!options.childElementCount) { const empty = document.createElement('span'); empty.className = 'provider-row-count'; empty.textContent = 'No model matches'; options.appendChild(empty); }
  }
  ...
  function renderLogin() {
    ...
    if (!providers.length) { box.appendChild(element('p', '', 'Refresh to list OMP login providers. Cedia does not open a browser until you ask.')); return; }
  ...
  function renderLoginRows(target) {
    const providers = state.loginProviders || [];
    if (!providers.length) { target.appendChild(element('p', '', 'Refresh to list OMP login providers. Cedia does not open a browser until you ask.')); return; }
    providers.forEach((provider) => {
      const row = element('div', 'login-row');
      const wrap = element('span'); wrap.appendChild(element('span', 'name', provider.name || provider.id)); wrap.appendChild(element('span', 'meta', provider.authenticated ? 'signed in' : (provider.available === false ? 'unavailable' : 'not signed in')));
      const button = element('button', 'ghost', provider.authenticated ? 'Re-login' : 'Log in'); button.type = 'button'; button.disabled = provider.available === false; button.addEventListener('click', () => post({ type: 'start_login', providerId: provider.id }));
  ...
  function renderCatalog() {
    ...
    const settingIndex = [
      ...
      { id: 'shortcuts', section: 'Appearance', label: 'Shortcuts', description: 'Advertised Cedia chords. Edit in Code-OSS Keyboard Shortcuts.' },
    ...
    const copy = {
      ...
      'Models/providers': 'Listed models come from the host registry. Unavailable rows stay visible. Cedia does not silently pick a billed fallback.',
      ...
      'Workspace/editor': 'Code-OSS owns Explorer, LSP, undo, and editor settings. Cedia does not replace the IDE.',
      ...
      'Notifications': 'In-app attention is on. System notifications stay off until a Cedia notification contract exists. Sound stays off.',
      'Privacy/security': 'Secrets stay redacted. Cedia does not export provider tokens. Diagnostics preview redaction before export when that flow exists.',
      'About/updates/licenses': 'Cedia.app is an ad-hoc personal build. Updates and notarization are not claimed.',
    }[settingsSection] || '';
    ...
    if (settingsSection === 'Notifications') {
      ...
      const system = element('button', '', 'System notifications: off'); system.type = 'button'; system.disabled = true; system.title = 'System notifications stay off until a Cedia notification contract exists.';
    ...
    if (settingsSection === 'Appearance') {
      ...
      page.appendChild(element('p', '', 'Shortcuts are advertised Cedia chords. Collision and remapping stay in Code-OSS Keyboard Shortcuts.'));
      const shortcuts = state.shortcutRows || [];
      if (!shortcuts.length) page.appendChild(element('p', 'settings-source', 'No advertised Cedia keybindings.'));
  ...
  function renderPresentations() {
    ...
    (state.presentations || []).forEach((item) => {
      if (!item || !item.method) return;
      if (item.method === 'open_url') {
        if (!item.url) return;
      ...
      if (item.method === 'notify') {
        ...
        const severity = item.notifyType === 'warning' || item.notifyType === 'error' ? item.notifyType : 'info';
        const card = element('section', 'presentation-notify');
        card.dataset.notifyType = severity;
        card.appendChild(element('p', '', item.message));
        box.appendChild(card);
        return;
      }
      if (item.method === 'setStatus') {
        if (!item.statusKey) return;
        const line = element('p', 'presentation-status', item.statusText ? item.statusKey + ': ' + item.statusText : item.statusKey);
        line.dataset.statusKey = item.statusKey;
        box.appendChild(line);
        return;
      }
      if (item.method === 'setWidget') {
        if (!item.widgetKey) return;
        const card = element('section', 'presentation-widget');
        card.dataset.widgetKey = item.widgetKey;
        card.appendChild(element('h4', '', item.widgetKey));
        (item.widgetLines || []).forEach((line) => card.appendChild(element('p', '', line)));
        box.appendChild(card);
        return;
      }
      if (item.method === 'setTitle') {
        if (!item.title) return;
        box.appendChild(element('p', 'presentation-title', 'Title: ' + item.title));
        return;
      }
      if (item.method === 'set_editor_text') {
        if (typeof item.text !== 'string') return;
        const card = element('section', 'presentation-editor-text');
        card.appendChild(element('pre', '', item.text));
        box.appendChild(card);
      }
    });
  }
  ...
  function renderPlan() {
    ...
    const actions = element('div', 'pref-row');
    [['Retry step', 'OMP has not advertised plan step retry. Cedia will not invent a retry owner.'], ['Cancel subagent', 'OMP has not advertised subagent cancel. Cedia will not stop a child run from this strip.'], ['Reorder', 'OMP has not advertised plan reorder.']].forEach((pair) => {
  ...
  function renderWorkTabs() {
    ...
    const copy = {
      changes: ['Changes', 'Tracked and untracked files for this task workspace. Stage and commit stay in Code-OSS until a Cedia review contract exists.', '', ''],
      ...
      artifacts: ['Artifacts', 'Immutable receipts with hash and build id. Cedia does not execute unknown MIME.', 'Open artifacts', 'artifacts'],
      files: ['Files', 'Code-OSS owns Explorer, search, LSP, and undo. Cedia does not replace the editor.', 'Files & editor', 'files'],
    ...
    if (active === 'terminal') {
      ...
      users.forEach((term) => {
        ...
        focus.title = state.userPtyFocusReason || 'Focus shows the Code-OSS terminal. Cedia does not embed a second PTY renderer.';
    ...
    if (active === 'preview') {
      ...
      if (lastGood) {
        ...
        const frame = element('div', 'preview-frame', lastGood.viewer === 'inline' ? 'Inline preview reserved. File is not executed.' : 'Unknown MIME is download only. Cedia will not execute this file.');
        ...
        if (retained && retained.incomingPreview) {
          ...
          switchB.title = 'Current build is download only. Cedia will not execute it.';
    ...
    if (active === 'changes') {
      ...
      if (review.stale) card.appendChild(element('p', '', review.staleReason || 'The file changed — reload the diff. Cedia will not apply a stale hunk.'));
      if (review.dirtyConflict) card.appendChild(element('p', '', review.dirtyConflictReason || 'The file has unsaved edits. Cedia will not apply a proposal over a dirty buffer.'));
      if (review.noGit) {
        card.appendChild(element('p', '', review.error || 'This folder is not a Git repository. Git setup is an explicit choice. Cedia does not initialize a repository from this panel.'));
        if (!(review.proposals || []).length) card.appendChild(element('p', '', 'No advertised agent proposals for this folder. Cedia does not invent hunks outside Git or OMP.'));
      }
      ...
      if (review.selectedPath) {
        ...
        if (selected && selected.status === 'conflict') {
          ...
          merge.title = 'Conflict. Cedia will not merge. Open the Code-OSS merge editor if Git advertised one.';
      ...
      bring.title = 'OMP has not advertised a checkpoint to restore. Cedia will not invent a bring-back.';
      ...
      ['Apply proposal', 'Discard proposed hunk', 'Stage hunk', 'Unstage hunk'].forEach((label) => {
        ...
        button.title = review.dirtyConflict
          ? (review.dirtyConflictReason || 'The file has unsaved edits. Cedia will not apply a proposal over a dirty buffer.')
      ...
      const commitBtn = element('button', 'primary', 'Commit'); commitBtn.type = 'button'; commitBtn.disabled = true; commitBtn.title = commit.commitReason || 'Commit stays in Code-OSS until OMP advertises a task-scoped commit contract. Cedia will not auto-push.';
    ...
    (state.visibleResources || []).filter((item) => item.status === 'expired').forEach((item) => {
      card.appendChild(element('p', '', (item.label || item.id) + ' expired — Cedia will not restart it automatically.'));
  ...
  function applyLayout() {
    ...
    if (width) document.documentElement.style.setProperty('--cedia-panel-w', width + 'px');
    if (height) document.documentElement.style.setProperty('--cedia-panel-h', height + 'px');
    document.documentElement.style.setProperty('--cedia-sidebar', sidebar + 'px');
    ...
    setMs('--cedia-motion-instant', motion.instant, 50);
    setMs('--cedia-motion-feedback', motion.feedback, 100);
    setMs('--cedia-motion-surface-in', motion.surfaceIn, 150);
    setMs('--cedia-motion-surface-out', motion.surfaceOut, 100);
    setMs('--cedia-motion-drawer-in', motion.drawerIn, 200);
    setMs('--cedia-motion-drawer-out', motion.drawerOut, 150);
  ...
  function bindSash() {
    ...
    const onMove = (event) => {
      if (vertical) document.documentElement.style.setProperty('--cedia-panel-h', Math.max(160, startH + (start - event.clientY)) + 'px');
      else document.documentElement.style.setProperty('--cedia-panel-w', Math.max(280, Math.min(640, startW + (start - event.clientX))) + 'px');
    };
    const onUp = (event) => {
  ...
  function bindSidebarSash() {
    ...
    const onMove = (event) => {
      document.documentElement.style.setProperty('--cedia-sidebar', Math.max(160, Math.min(360, startW + (event.clientX - start))) + 'px');
    };
    const onUp = (event) => {
      window.removeEventListener('pointermove', onMove);
      window.removeEventListener('pointerup', onUp);
  ...
  function renderRouteError() {
    ...
    text(byId('route-error-body'), err.body || 'This task is missing. Cedia will not open a different task.');
  ...
  function renderBranchMenu() {
    ...
    menu.appendChild(element('p', '', branch ? ('Current · ' + branch + '. Selecting a ref targets the new draft. Cedia will not checkout until you confirm a workspace workflow.') : 'No Git branch until the folder has a repository. New worktree stays unavailable.'));
  ...
  }); const modelTrigger = byId('model-select'); const modelPanel = byId('model-panel'); const modelSearch = byId('model-search');
  const closeModelPanel = () => { if (!modelPanel || !modelTrigger) return; modelPanel.hidden = true; modelTrigger.setAttribute('aria-expanded', 'false'); };
  if (modelTrigger && modelPanel) {
    modelTrigger.addEventListener('click', () => { const opening = modelPanel.hidden; if (opening) { const current = (state.models || []).filter((model) => model.id === (state.selectedModel || ''))[0]; if (current) activeProvider = providerLabel(current); renderModels(); } modelPanel.hidden = !opening; modelTrigger.setAttribute('aria-expanded', opening ? 'true' : 'false'); if (opening && modelSearch) { modelSearch.value = ''; renderModels(); modelSearch.focus(); } });
    modelPanel.addEventListener('click', (event) => {
      const provider = event.target.closest('[data-provider]');
      if (provider) { activeProvider = provider.dataset.provider; renderModels(); return; }
      const row = event.target.closest('[data-model-id]');
      if (!row || row.disabled) return;
      closeModelPanel();
      post({ type: 'select_model', modelId: row.dataset.modelId });
    });
    if (modelSearch) modelSearch.addEventListener('input', () => renderModels());
    if (modelSearch) modelSearch.addEventListener('keydown', (event) => { if (event.key !== 'Enter') return; event.preventDefault(); const box = byId('model-options'); const first = box && Array.from(box.querySelectorAll('[data-model-id]')).filter((row) => !row.disabled)[0]; if (first) { closeModelPanel(); post({ type: 'select_model', modelId: first.dataset.modelId }); } });
    document.addEventListener('keydown', (event) => { if (event.key === 'Escape' && !modelPanel.hidden) { closeModelPanel(); modelTrigger.focus(); } });
    document.addEventListener('click', (event) => { if (!modelPanel.hidden && !event.target.closest('.model-picker')) closeModelPanel(); });
  }
  ...
  window.addEventListener('message', (event) => { const message = event.data || {}; if (message.type === 'snapshot' || message.type === 'state') { const next = message.state || {}; const nextId = next.session && next.session.id; if (nextId !== lastSessionId) closeTransientMenus(); lastSessionId = nextId; Object.assign(state, next); render(); } else if (message.type === 'prefill') { const input = byId('composer-input'); input.value = message.text || ''; state.draft = input.value; input.focus(); } else if (message.type === 'error') { state.lastError = message.text || 'Cedia host error'; state.connection = message.status || 'unknown'; renderStatus(); } else if (message.type === 'focus_composer') byId('composer-input').focus(); else if (message.type === 'focus_search') { const search = byId('task-search'); if (search) { search.focus(); search.select(); } } else if (message.type === 'focus_task') { const heading = byId('task-title'); if (heading) { heading.setAttribute('tabindex', '-1'); heading.focus(); } } else if (message.type === 'open_settings') openSettings(message.section || 'Devices/connections'); });
  ...
  <main class="main is-empty" id="main" aria-label="Task">
    ...
    <div class="composer">
      <div class="composer-box">
        ...
        <textarea id="composer-input" rows="3" aria-label="Message Cedia" placeholder="Plan, Build, / for skills, @ for context"></textarea>
        ...
        <div class="composer-bar">
          ...
          <div class="model-picker">
            <button type="button" id="model-select" class="model-select" aria-label="Model" aria-haspopup="listbox" aria-expanded="false" disabled><span id="model-chip" class="provider-chip" aria-hidden="true"></span><span id="model-label" class="model-label">Model</span><span class="model-chev" aria-hidden="true"></span></button>
            <div id="model-panel" class="model-panel" hidden>
              <input id="model-search" class="model-panel-search" type="search" placeholder="Search models or providers" aria-label="Search models or providers" />
              <div class="model-columns">
                <div id="model-providers" class="provider-column" role="listbox" aria-label="Providers"></div>
                <div id="model-options" class="model-column" role="listbox" aria-label="Models"></div>
              </div>
            </div>
          </div>
      ...
      <div class="mode-pills">
        ...
        <button type="button" class="pill" disabled title="Cloud is not a Cedia runtime.">Run in Cloud</button>
      </div>
...
<div class="route-error" id="route-error" hidden>
  <div class="route-error-card">
    ...
    <p id="route-error-body">This task is missing. Cedia will not open a different task.</p>

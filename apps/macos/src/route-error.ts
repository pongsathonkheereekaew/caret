 ...
 * Deep links validate the requested project/session/resource. Caret never opens a nearby task instead. */
...
function pageBody(input: RouteErrorPageInput): string {
	...
	if (input.reason === "unknown") {
		return `Caret does not know why this ${KIND_NOUN[input.kind]}${idNote} cannot be opened. It will not guess a nearby destination.`;
	}
	if (input.kind === "devices" && input.reason === "revoked") {
		return `This device${idNote} is revoked. Caret will not auto-pair.`;
	}
	if (input.kind === "task") {
		const state = input.reason === "deleted" ? "was deleted" : input.reason === "revoked" ? "is revoked" : "is missing";
		return `This task${idNote} ${state}. Caret will not open a different task.`;
	}
	const state = input.reason === "deleted" ? "was deleted" : input.reason === "revoked" ? "is revoked" : "is missing";
	return `This ${KIND_NOUN[input.kind]}${idNote} ${state}. Caret will not open a nearby destination instead.`;
}

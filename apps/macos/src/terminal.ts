...
const HISTORY_TRUNCATED_MARKER = "\r\n[cedia] Terminal history was truncated while this panel was closed. The retained tail follows; resizing requested a fresh redraw.\r\n";
...
export class OmpTerminalViews {
	...
	ingest(event: SessionEvent): void {
		...
		if (frame.type === "cedia_terminal_open") {
			if (!record) {
				record = {
		...
		if (frame.type === "cedia_terminal_output" && typeof frame.data === "string" && typeof sequence === "number" && Number.isSafeInteger(sequence) && sequence > record.sequence) {
		...
		if (frame.type === "cedia_terminal_close") {
			if (record.ended) return;
			record.ended = true;
			if (record.opened && !record.closed) record.write?.fire("\r\n[Cedia interaction ended]\r\n");
		}
	}
		...
		const pty: vscode.Pseudoterminal = {
			...
			open: dimensions => {
				...
				if (nextDimensions) {
					...
					send("cedia_terminal_resize", { cols: nextDimensions.columns, rows: nextDimensions.rows });
				}
			...
			handleInput: data => {
				...
				send("cedia_terminal_input", { data });
			},
...
export const CEDIA_TERMINAL_HISTORY_LIMIT_BYTES = MAX_HISTORY_BYTES;
export const CEDIA_TERMINAL_HISTORY_TRUNCATED_MARKER = HISTORY_TRUNCATED_MARKER;

/** In-editor marks for the edits Caret applied to an open buffer.
 *
 ...
 * the Cursor-class affordance on top of that - show which regions Caret
...
export function agentEditLabel(path: string, ranges: readonly MarkRange[]): string {
	...
	return `${path}: ${regions}, ${lines} ${lines === 1 ? "line" : "lines"} changed by Caret`;
}
...
/** Hover text on every Caret mark, naming the file and the two actions. */
export function decorationHover(path: string): string {
	return `Caret applied this edit to ${path}. Save to keep it, or run "Caret: Take Back the Caret Edit".`;
}

/** A Caret edit waiting on the user's decision. */
...
export const STALE_AGENT_EDIT_REASON =
	"The file changed after Caret's edit, so taking that edit back here would also undo your own change. Use Undo instead.";
 ...
 * while the buffer still holds the version Caret produced. Once anything else
 * has touched the buffer, the user's own work is in there too, so Caret
...
export const AGENT_EDIT_REVIEW_STALE_REASON =
	"The file changed after Caret's edit, so a diff against the pre-edit text would also show your own change. Use Undo to compare instead.";

export type AgentEditReviewDecision =
	| { readonly ok: true }
	| { readonly ok: false; readonly reason: string };

/** Whether a review diff of *only* Caret's change is honest to open. The left
 * side is the recorded pre-edit text, so once anything else has touched the
 * buffer the diff would misattribute the user's own change to Caret; Caret
...
/** The two decisions, placed on the first region Caret changed, the way a
...
export function agentEditLenses(pending: PendingAgentEdit): AgentEditLensSpec[] {
	...
	return [
		{ line, title: "Keep the Caret Edit", command: "caret.keepAgentEdit" },
		{ line, title: "Take Back the Caret Edit", command: "caret.revertAgentEdit" },
	];
}

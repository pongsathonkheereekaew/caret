...
import {
  CEDIA_TERMINAL_MAX_HISTORY_BYTES,
...
describe("OMP virtual terminal projection", () => {
  test("keeps ANSI output ordered and idempotent across duplicate replay", () => {
    let terminals = applyVirtualTerminalFrame([], frame("cedia_terminal_open", { terminalId: "tty-1", title: "Shell", cols: 80, rows: 24 }));
    terminals = applyVirtualTerminalFrame(terminals, frame("cedia_terminal_output", { terminalId: "tty-1", sequence: 0, data: "\u001b[31mred\u001b[0m" }));
    terminals = applyVirtualTerminalFrame(terminals, frame("cedia_terminal_output", { terminalId: "tty-1", sequence: 1, data: "\nnext" }));
    const duplicate = applyVirtualTerminalFrame(terminals, frame("cedia_terminal_output", { terminalId: "tty-1", sequence: 1, data: "MUTATION" }));
  ...
  test("rejects output before open and after close, then preserves history on reopen", () => {
    let terminals = applyVirtualTerminalFrame([], frame("cedia_terminal_output", { terminalId: "tty-1", sequence: 0, data: "late" }));
    expect(terminals).toHaveLength(0);
    terminals = applyVirtualTerminalFrame([], frame("cedia_terminal_open", { terminalId: "tty-1", cols: 100, rows: 30 }));
    terminals = applyVirtualTerminalFrame(terminals, frame("cedia_terminal_output", { terminalId: "tty-1", sequence: 0, data: "old" }));
    terminals = applyVirtualTerminalFrame(terminals, frame("cedia_terminal_close", { terminalId: "tty-1", reason: "stopped" }));
    terminals = applyVirtualTerminalFrame(terminals, frame("cedia_terminal_output", { terminalId: "tty-1", sequence: 1, data: "must-not-resurrect" }));
    expect(terminals[0]?.outputs.map(item => item.data)).toEqual(["old"]);
    terminals = applyVirtualTerminalFrame(terminals, frame("cedia_terminal_open", { terminalId: "tty-1", cols: 90, rows: 20 }));
    terminals = applyVirtualTerminalFrame(terminals, frame("cedia_terminal_output", { terminalId: "tty-1", sequence: 1, data: "new" }));
  ...
  test("bounds history while retaining the newest sequence", () => {
    let terminals = applyVirtualTerminalFrame([], frame("cedia_terminal_open", { terminalId: "tty-1", cols: 80, rows: 24 }));
    const chunk = "x".repeat(64 * 1024);
    for (let sequence = 0; sequence < 80; sequence += 1) {
      terminals = applyVirtualTerminalFrame(terminals, frame("cedia_terminal_output", { terminalId: "tty-1", sequence, data: chunk }));
    }
    expect(terminals[0]?.historyBytes).toBeLessThanOrEqual(CEDIA_TERMINAL_MAX_HISTORY_BYTES);
  ...
  test("validates durable input and resize payloads without retry metadata", () => {
    expect(terminalInputCommand("tty-1", "\r")).toEqual({ command: "cedia_terminal_input", payload: { terminalId: "tty-1", data: "\r" } });
    expect(terminalResizeCommand("tty-1", 120, 32)).toEqual({ command: "cedia_terminal_resize", payload: { terminalId: "tty-1", cols: 120, rows: 32 } });
    expect(() => terminalInputCommand("tty-1", "x".repeat(64 * 1024 + 1))).toThrow();
    expect(() => terminalResizeCommand("tty-1", 1, 32)).toThrow();
    expect(terminalNegotiateCommand(120, 32)).toEqual({ command: "cedia_terminal_negotiate", payload: { version: 1, cols: 120, rows: 32 } });
...
const checkpoint: TerminalCheckpoint = {
  ...
  cursorRow: 2,
  cursorCol: 5,
  lines: ["$ npm test", "cedia-virtual-output"],
  lastSequence: 41,
  closed: false,
...
describe("host terminal checkpoint", () => {
  test("paints the host grid and puts the cursor back", () => {
    expect(terminalCheckpointSeed(checkpoint)).toBe(
      "\u001b[?25l\u001b[2J\u001b[H"
      + "\u001b[1;1H$ npm test\u001b[K"
      + "\u001b[2;1Hcedia-virtual-output\u001b[K"
      + "\u001b[3;6H\u001b[?25h",
    );
  });

  test("paints styled runs in true colour and resets each styled row", () => {
    const styled: TerminalCheckpoint = {
      ...checkpoint,
      lines: ["ok", "warn", ""],
      cursorRow: 0,
      cursorCol: 0,
      runs: [
        { row: 0, col: 2, text: "ok", foreground: "#00ff00" },
        { row: 1, col: 0, text: "!", bold: true, background: "#112233" },
        { row: 1, col: 2, text: "x", italic: true, underline: true, foreground: "#ff0000", background: "#000000" },
        { row: 9, col: 0, text: "outside" },
        { row: 2, col: 0, text: "" },
      ],
    };
    expect(terminalCheckpointSeed(styled)).toBe(
      "\u001b[?25l\u001b[2J\u001b[H"
      + "\u001b[1;3H\u001b[0m\u001b[38;2;0;255;0mok\u001b[0m\u001b[K"
      + "\u001b[2;1H\u001b[0m\u001b[1;48;2;17;34;51m!\u001b[2;3H\u001b[0m\u001b[3;4;38;2;255;0;0;48;2;0;0;0mx\u001b[0m\u001b[K"
      + "\u001b[3;1H\u001b[K"
      + "\u001b[1;1H\u001b[?25h",
    );
  });

  test("skips unparseable colours instead of inventing SGR", () => {
    const styled: TerminalCheckpoint = {
      ...checkpoint,
      lines: ["plain"],
      cursorRow: 0,
      cursorCol: 0,
      runs: [{ row: 0, col: 0, text: "plain", foreground: "red", background: "#12345" }],
    };
    expect(terminalCheckpointSeed(styled)).toBe(
      "\u001b[?25l\u001b[2J\u001b[H"
      + "\u001b[1;1H\u001b[0mplain\u001b[0m\u001b[K"
...
describe("bundled terminal document bridge", () => {
  ...
  async function bootDocument(): Promise<{ writes: string[]; send: (payload: Record<string, unknown>) => void }> {
    ...
    const send = (payload: Record<string, unknown>) => {
      const event = { data: JSON.stringify({ source: "cedia-terminal", payload }) };
  ...
  test("contains xterm bundle and only accepts the Cedia bridge envelope", () => {
    const html = terminalDocument({ terminalId: "tty-1", cols: 80, rows: 24, title: "Shell" });
    ...
    expect(terminalMessage(JSON.stringify({ source: "cedia-terminal", payload: { type: "ready", terminalId: "tty-1" } }))).toMatchObject({ type: "ready", terminalId: "tty-1" });

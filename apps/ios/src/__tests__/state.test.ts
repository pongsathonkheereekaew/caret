...
describe("mobile transcript projection", () => {
  ...
  test("projects login URLs as presentations and never treats javascript as openable", () => {
    let state = createInitialMobileState({ session });
    state = applyMobileEvent(state, event(1, { type: "caret_ui", event: { kind: "presentation", request: { id: "url-1", method: "open_url", url: "https://example.test/login", instructions: "Open manually" } } }));
    ...
    state = applyMobileEvent(state, event(2, { type: "caret_ui", event: { kind: "presentation", request: { id: "bad", method: "open_url", url: "javascript:alert(1)" } } }));
  ...
  test("keeps interactive events until explicitly resolved", () => {
    let state = createInitialMobileState({ session });
    state = applyMobileEvent(state, event(1, { type: "caret_ui", event: { kind: "interactive", token: "t1", request: { method: "select", id: "i1", title: "Choose", options: ["A", "B"] } } }));
  ...
  test("syncs live UI broker state without replacing same-token requests", () => {
    let state = createInitialMobileState({ session });
    const eventValue = event(1, { type: "caret_ui", event: { kind: "interactive", token: "t1", request: { method: "input", id: "i1", title: "Name", placeholder: "Project" } } });

...
import { createCachedSnapshot, parseCachedSnapshot, readCachedSnapshot, restoreCachedSnapshot, snapshotKey, writeCachedSnapshot, type SnapshotCache } from "../core/replay.ts";
...
function memoryCache(): SnapshotCache & { values: Map<string, string> } { const values = new Map<string, string>(); return { values, get: async key => values.get(key) ?? null, set: async (key, value) => { values.set(key, value); } }; }

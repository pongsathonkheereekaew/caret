...
${XTERM_CSS}
html,body,#cedia-terminal{width:100%;height:100%;margin:0;overflow:hidden;background:#10131b;color:#f4f6fb}
body{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace}
#cedia-terminal{padding:10px;box-sizing:border-box}
.xterm-viewport{overflow-y:auto!important}
</style></head><body><div id="cedia-terminal" role="application" aria-label="${options.title ? options.title.replace(/[&<>"']/g, "") : "Terminal"}" tabindex="0"></div>
...
function post(message){
  var text=JSON.stringify({source:"cedia-terminal",payload:message});
...
function safeMessage(event){
  ...
  if(value&&value.source==="cedia-terminal") value=value.payload;
...
try{
  ...
  terminal.open(document.getElementById("cedia-terminal"));
  installParserGuards();
...
export function terminalMessage(value: unknown): { type: string; [key: string]: unknown } | null {
  ...
  try {
    ...
    const payload = record.source === "cedia-terminal" ? record.payload : parsed;

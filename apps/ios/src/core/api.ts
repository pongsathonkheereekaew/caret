...
function objectBody<T>(body: unknown, description: string): T {
  if (!isRecord(body)) throw new Error(`Caret host returned an invalid ${description}`);
  return body as T;
}

function arrayBody<T>(body: unknown, description: string): T[] {
  if (!Array.isArray(body)) throw new Error(`Caret host returned an invalid ${description}`);
  return body as T[];
}

export interface CaretApiOptions {
...
interface FrameReference {
  readonly type: "caret_frame_reference";
...
function frameReference(value: unknown): value is FrameReference {
  ...
  if (value.type !== "caret_frame_reference" || !Number.isSafeInteger(value.sequence) || !Number.isSafeInteger(length) || (length as number) < 0 || (length as number) > MAX_FRAME_TEXT_CHARS || typeof sha256 !== "string" || !/^[a-f0-9]{64}$/i.test(sha256)) return false;
  return true;
}
...
export class CaretApi {
  readonly #transport: ClientTransport;
  readonly #digestSha256: (text: string) => Promise<string>;

  constructor(options: CaretApiOptions) {
  ...
  async request<T>(method: TransportMethod | string, path: string, body?: unknown): Promise<T> {
    ...
    if (result.status < 200 || result.status >= 300) throw new Error(`Caret host request failed (${result.status})`);
  ...
  async #hydrateResponseBody(path: string, body: unknown): Promise<unknown> {
    if (!isRecord(body) || !responseReference(body.caretResponseReference)) return body;
    return this.#readResponseText(body.caretResponseReference, path);
  }
  ...
  async #readResponseText(reference: ResponseReference, sourcePath: string): Promise<Json> {
    ...
    while (offset < reference.length) {
      ...
      if (result.status < 200 || result.status >= 300) throw new Error(`Caret response reference fetch failed (${result.status})`);
      ...
      if (typeof chunkHash !== "string" || chunkHash.toLowerCase() !== reference.sha256.toLowerCase() || chunkLength !== reference.length || chunkOffset !== offset || typeof chunk !== "string") {
        throw new Error(`Caret response reference for ${sourcePath} is invalid`);
      }
      if (chunk.length === 0 || chunk.length > MAX_FRAME_CHUNK_CHARS || offset + chunk.length > reference.length) {
        throw new Error(`Caret response reference for ${sourcePath} has an invalid chunk length`);
      }
      text += chunk;
      offset += chunk.length;
      if (text.length > MAX_FRAME_TEXT_CHARS) throw new Error("Caret response exceeds the mobile size limit");
    }
    if (text.length !== reference.length) throw new Error(`Caret response reference for ${sourcePath} is truncated`);
    const digest = (await this.#digestSha256(text)).toLowerCase();
    if (digest !== reference.sha256.toLowerCase()) throw new Error(`Caret response reference for ${sourcePath} failed integrity verification`);
    try {
      return JSON.parse(text) as Json;
    } catch (error) {
      throw new Error(`Caret response reference for ${sourcePath} is malformed`, { cause: error });
    }
  }
  ...
  async #hydrateEventPage(sessionId: string, page: EventPage): Promise<EventPage> {
    ...
    for (const event of page.events) {
      ...
      try {
        ...
        throw new Error(`Caret host returned malformed event frame ${event.sequence}`, { cause: error });
      }
  ...
  async #readFrameText(sessionId: string, sequence: number, reference: FrameReference): Promise<string> {
    ...
    while (offset < reference.length) {
      ...
      if (chunkSequence !== sequence || chunkOffset !== offset || typeof chunk !== "string" || !Number.isSafeInteger(chunkLength) || chunkLength !== reference.length || typeof chunkHash !== "string" || chunkHash.toLowerCase() !== reference.sha256.toLowerCase()) {
        throw new Error(`Caret host returned an invalid event frame chunk ${sequence}`);
      }
      if (chunk.length === 0 || chunk.length > MAX_FRAME_CHUNK_CHARS || offset + chunk.length > reference.length) {
        throw new Error(`Caret host returned an invalid event frame chunk length ${sequence}`);
      }
      text += chunk;
      offset += chunk.length;
      if (text.length > MAX_FRAME_TEXT_CHARS) throw new Error("Caret event frame exceeds the mobile size limit");
    }
    if (text.length !== reference.length) throw new Error(`Caret event frame ${sequence} is truncated`);
    const digest = (await this.#digestSha256(text)).toLowerCase();
    if (digest !== reference.sha256.toLowerCase()) throw new Error(`Caret event frame ${sequence} failed integrity verification`);
    return text;
  }
  ...
  /** Authoritative live UI broker state; logged historical caret_ui frames are not actionable. */

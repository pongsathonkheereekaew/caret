...
export const CARET_HOST_ARTIFACT_MAX_BYTES = 256 * 1024 * 1024;
/**
 ...
 */
export const CARET_MOBILE_ARTIFACT_MAX_BYTES = 32 * 1024 * 1024;
export const CARET_MOBILE_TEXT_PREVIEW_MAX_BYTES = 2 * 1024 * 1024;
export const CARET_ARTIFACT_CHUNK_MAX_BYTES = 96 * 1024;
export const CARET_ARTIFACT_MAX_SOURCE_HASHES = 1_000;
...
export function parseArtifactReceipt(value: unknown, expectedSessionId?: string): ArtifactReceipt {
  if (!record(value)) throw new Error("Caret host returned an invalid artifact receipt");
  ...
  if (!Number.isSafeInteger(size) || (size as number) < 0 || (size as number) > CARET_HOST_ARTIFACT_MAX_BYTES) throw new Error("Invalid artifact size");
  const sourceHashes = value.sourceHashes;
  if (!Array.isArray(sourceHashes) || sourceHashes.length > CARET_ARTIFACT_MAX_SOURCE_HASHES) throw new Error("Invalid artifact source hashes");
...
function decodeBase64(value: string): Uint8Array {
  try {
    ...
    if (bytes.byteLength > CARET_ARTIFACT_CHUNK_MAX_BYTES) throw new Error("Artifact chunk exceeds 96 KiB");
    return bytes;
  } catch (error) {
...
export function parseArtifactChunk(value: unknown, sessionId: string, sha256: string, expectedOffset: number): ArtifactChunk {
  ...
  if (!record(value)) throw new Error("Caret host returned an invalid artifact chunk");
  ...
  if (byteLength > CARET_ARTIFACT_CHUNK_MAX_BYTES) throw new Error("Artifact chunk exceeds 96 KiB");
...
export async function readArtifactBytes(reader: ArtifactReader, receipt: ArtifactReceipt, options: ArtifactTransferOptions = {}): Promise<Uint8Array> {
  ...
  const bound = options.maxBytes ?? CARET_MOBILE_ARTIFACT_MAX_BYTES;
  if (!Number.isSafeInteger(bound) || bound < 0 || bound > CARET_HOST_ARTIFACT_MAX_BYTES) throw new RangeError("Invalid mobile artifact limit");
...
export async function downloadArtifact(receipt: ArtifactReceipt, bytes: Uint8Array, platform: "web" | "native"): Promise<ArtifactDownload> {
  ...
  if (bytes.byteLength > CARET_MOBILE_ARTIFACT_MAX_BYTES) throw new RangeError("Artifact exceeds the mobile download limit");
  ...
  const directory = new fileSystem.Directory(fileSystem.Paths.cache, "caret-artifacts");

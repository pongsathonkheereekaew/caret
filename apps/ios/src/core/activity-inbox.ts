...
export function activityInboxTapAction(item: ActivityInboxItem): ActivityInboxTapAction {
  const sessionId = activityInboxItemSessionId(item);
  if (!sessionId) {
    return { type: "unavailable", reason: "This item has no session id, so it cannot be opened. Caret will not invent a task." };
  }
...
export function activityInboxBody(item: ActivityInboxItem, sessionTitle?: string): string {
  if (item.kind === "unknown_command") {
    if (!activityInboxItemSessionId(item)) return "This unknown outcome has no session id, so it cannot be opened. Caret will not invent a task or treat this as an approval.";
    if (!sessionTitle) return "This unknown outcome names a task that is not in the loaded list. Caret will not invent a session or treat this as an approval.";
    return "Same Mac session — open the matching task. This is an unknown command outcome, not an approval. Caret will not answer or replay it from this list.";
  }
  if (!activityInboxItemSessionId(item)) return "This request has no session id, so it cannot be opened. Caret will not invent a task. Approvals are not answered from this list.";
  if (!sessionTitle) return "This request names a task that is not in the loaded list. Caret will not invent a session. Approvals are not answered from this list automatically.";

...
describe("TerminalStateRegistry", () => {
  it("keeps one screen per terminal from open, output and close frames", async () => {
    ...
    registry.apply({ type: "caret_terminal_open", terminalId: "t1", cols: 40, rows: 4, title: "zsh" });
    registry.apply({ type: "caret_terminal_output", terminalId: "t1", sequence: 0, data: "hello\n" });
    registry.apply({ type: "caret_terminal_output", terminalId: "t1", sequence: 1, data: "world\n" });
    ...
    registry.apply({ type: "caret_terminal_close", terminalId: "t1", reason: "exited" });
  ...
  it("ignores duplicate sequences and marks a real gap as incomplete", async () => {
    const registry = registryWithFakeEngine();
    registry.apply({ type: "caret_terminal_open", terminalId: "t1", cols: 40, rows: 4 });
    registry.apply({ type: "caret_terminal_output", terminalId: "t1", sequence: 0, data: "one\n" });
    registry.apply({ type: "caret_terminal_output", terminalId: "t1", sequence: 0, data: "duplicate\n" });
    await settle(registry, 1);
    expect(registry.snapshots()[0].lines).toEqual(["one"]);

    registry.apply({ type: "caret_terminal_output", terminalId: "t1", sequence: 4, data: "after-gap\n" });
    const [checkpoint] = registry.snapshots();
  ...
  it("reopens a terminal for a reattaching client without losing the screen", async () => {
    const registry = registryWithFakeEngine();
    registry.apply({ type: "caret_terminal_open", terminalId: "t1", cols: 40, rows: 4 });
    registry.apply({ type: "caret_terminal_output", terminalId: "t1", sequence: 0, data: "kept\n" });
    registry.apply({ type: "caret_terminal_close", terminalId: "t1", reason: "detached" });
    registry.apply({ type: "caret_terminal_open", terminalId: "t1", cols: 80, rows: 10 });
    await settle(registry, 1);
  ...
  it("keeps running without an engine and reports no checkpoints", async () => {
    ...
    registry.apply({ type: "caret_terminal_open", terminalId: "t1", cols: 40, rows: 4 });
    registry.apply({ type: "caret_terminal_output", terminalId: "t1", sequence: 0, data: "unused\n" });
...
describe("libghostty-vt (the host's terminal engine)", () => {
  ...
  it("loads a prebuild for this platform and renders a real screen", async () => {
    ...
    registry.apply({ type: "caret_terminal_open", terminalId: "t1", cols: 40, rows: 6 });
    registry.apply({ type: "caret_terminal_output", terminalId: "t1", sequence: 0, data: "line one\r\n\x1b[31mred\x1b[0m ไทย\r\n" });
    await settle(registry, 1);

...
function referenceOf(value: unknown): { sha256: string; length: number } {
	if (!value || typeof value !== "object" || !("caretResponseReference" in value)) throw new Error("Expected a response reference");
	const reference = (value as { caretResponseReference: { sha256: unknown; length: unknown } }).caretResponseReference;

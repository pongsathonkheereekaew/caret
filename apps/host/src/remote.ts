...
import { createCaretRelayPairingOffer, type CaretRelayJson } from "../../../packages/relay/src/protocol.ts";
...
export class RemoteConnection {
  ...
  async pair(name: string) {
    ...
    return createCaretRelayPairingOffer({ serverId: this.#identity.serverId, daemonPublicKeyB64: this.#identity.publicKey,
  ...
  async #start() {
    if (!this.#server) this.#server = new CaretRelayServer({ endpoint: this.#identity.endpoint, useTls: true, serverId: this.#identity.serverId,
      ...
      handler: async request => { const response = await this.#router(request); return { status: response.status, body: JSON.parse(JSON.stringify(response.body)) as CaretRelayJson }; },

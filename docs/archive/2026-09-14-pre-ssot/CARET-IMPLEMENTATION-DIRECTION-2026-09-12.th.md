...
| G5 — Product acceptance/release | E1–E4 (limiter/E3 DAW **ไม่บังคับ** สำหรับ personal use), installer/update/migration/a11y | Receipts ตาม platform ที่ทดสอบจริง; full OMP matrix ของ pinned version ผ่าน; เทียบ Codex/Cursor ที่ชั้น agent+IDE+composer+MCP ไม่ใช่ Tab/cloud weights; open limitations แสดงชัด |
...
- [OMP control-plane probe](../../maintenance/evidence/omp-rpc-2026-09-12/README.md): root รัน `rpc` และ `rpc-ui` ซ้ำบน binary 18.1.18 พร้อม version/SHA256 receipt; exit 0, 34 frames ต่อโหมด, v2 negotiation และ configuration readback ผ่าน. Isolated registry มี 11 และ 12 tools ตามลำดับ ไม่ใช่จำนวน core tools ทั้งหมด
- [Static inventory](../../maintenance/evidence/omp-rpc-2026-09-12/source-inventory.json): source มี 28 built-in names, 3 hidden names, 42 RPC commands; Paseo schema มี 16 commands และมี shared negotiation/raw sends นอก schema. ไม่ใช้ตัวเลขนี้เป็นเปอร์เซ็นต์ full support

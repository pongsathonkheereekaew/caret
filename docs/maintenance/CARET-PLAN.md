...
|---|---|---|
| **D1 New Chat creates a draft** | Checked again 2026-09-17, and every route ends at the same place: the base's new-session flow always finishes in `ISessionsProvider.createNewSession`. `NewChatInSessionsWindowAction` passes the active session's folder into `openNewSession` → `_resolveProviderForNewSession` → the provider; the folder-less route shows the composer, whose own folder picker calls `openNewSession({ folderUri })` again → the provider; and the first Send materializes through `chatServiceImpl.acceptInput` → `createNewChatSessionItem` → the extension's `newChatSessionItemHandler`, which exists but always creates in the first non-archived project and ignores the composer's chosen workspace. Caret's provider cannot honour any of them today: `resolveWorkspace` accepts only the window's own folders and `createNewSession` returns the newest contributed session or throws `UNSUPPORTED` (the extensions provider's own comment says the draft is meant to be the newest contributed session). So the slice is three pieces, not one: (1) a draft the provider can return synchronously - the interface cannot await the extension - which is what `agentHostUntitledProvisionalSessionService` does for the agent host; (2) the chosen workspace reaching creation instead of 'first project'; (3) §10 item 3, because an empty Caret session currently shows the base's welcome rather than Caret's chat view. A nav-row-only change was considered and rejected: it would show a composer whose folder picker and Send both misbehave. | clicking `New Chat` in the Agents window produces a Caret draft whose workspace is the one the user chose, and the comparison's 24 state misses can be compared fairly against the reference's empty draft |

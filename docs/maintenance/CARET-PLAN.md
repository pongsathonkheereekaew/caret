...
|---|---|---|
| **D1 New Chat creates a draft** | **Seam found 2026-09-17; the port of `agentHostUntitledProvisionalSessionService` is not needed.** The base already owns untitled drafts end to end: `getNewChatSessionResource(sessionType)` builds `URI.from({ scheme: sessionType, path: '/untitled-<uuid>' })` (`chatUri.ts`), `isUntitledChatSession()` recognises it, and on the first Send `chatServiceImpl._materializeUntitledSession()` snapshots those options, calls `createNewChatSessionItem(type, { prompt, initialSessionOptions, untitledResource })` - which Caret's extension already implements - then aliases the real resource onto the untitled one and continues the conversation there. So the work is three small pieces and one decision: (1) Caret's provider (`extensionSessionsProvider.contribution.ts`, patch `0010`) returns a **draft** from `createNewSession` instead of returning the newest contributed session or throwing - an `ExtensionSession` whose chat resource is the untitled URI for `caret.omp` and whose workspace is the folder the caller passed; (2) the extension's `newChatSessionItemHandler` honours the composer's chosen workspace from `initialSessionOptions` instead of always taking the first non-archived project; (3) the reference's starter cards, which are the Agent Home's own surface (patch `0023`) and follow wherever the draft lands. The decision: whether the draft's workspace picker is the base's unified workspace picker (`sessions.chat.unifiedWorkspacePicker.enabled`) or Caret's own, because that is what decides where piece (2) reads the folder from. | clicking `New Chat` in the Agents window shows Caret's own empty draft (Caret welcome + composer), and the first Send creates a real session in the folder the user chose |

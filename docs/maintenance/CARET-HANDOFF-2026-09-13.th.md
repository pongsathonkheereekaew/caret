...
4. ตรวจ packaged Caret UI ใหม่เมื่อ Keychain พร้อม; rebuild package ให้ตรง runtime ล่าสุดก่อน acceptance. ใช้ private fixture/profile และตั้ง `caret.hostStateDir` ใน VS Code user settings จริง — env `CARET_STATE_DIR` อย่างเดียวไม่เปลี่ยน extension descriptor path. ห้ามไปใช้ default user host เป็น fixture
5. ทำ G4 physical iPhone/cellular/background/restart/revoke และ G5 E1–E4/limiter DAW/installer/update/migration เมื่อ prerequisite พร้อม. ยังมีงานโค้ดที่ไม่ขึ้นกับ prerequisite ให้ทำต่อ ไม่หยุดถาม “continue?” หลังแต่ละ slice
...
- macOS Keychain: UI เคยค้างจริงใน `SecItemCopyMatching` (`/tmp/caret-unresponsive-main.sample.txt`). CUA **ปฏิเสธ `com.apple.SecurityAgent` ด้วย safety reason**. ได้ขอผู้ใช้จัดการ prompt ด้วยตนเองแล้ว; ห้าม bypass/ใช้ plaintext/mock keychain/เครื่องมืออื่นกดแทน. ณ handoff process fixtureเก่า PID16814/18084 ไม่อยู่แล้ว แต่ **ยังไม่ได้ตรวจว่า UI กลับมาปกติ**. Session ใหม่ตรวจแอป Caret ได้ตามปกติ; ถ้าพบ prompt ให้ผู้ใช้จัดการ ไม่ถือว่า gate ผ่านจาก PID หาย
- CUA เคยปฏิเสธ Codex `com.openai.codex`; อย่า bypass ไปอ่านแอปเพื่อ reference. ใช้ official public refs/ตรวจแอป Caret ของเรา
- มี CommandLineTools แต่ไม่มี Xcode/iOS SDK/device tooling. ผู้ใช้ยังไม่ได้ตอบคำถามเตรียม Xcode/iPhone และ limiter path/DAW. ไม่อ้าง web export เป็น native acceptance

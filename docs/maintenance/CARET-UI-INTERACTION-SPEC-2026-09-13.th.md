...
- Desktop ≥1200 logical px: sidebar 240px ปรับ 200–360px; transcript อยู่กลาง max-width 900px; work panel เปิดเมื่อเลือก resource กว้างเริ่มต้น 360px ปรับ 280–640px โดยต้องเหลือ main ≥360px. เมื่อไม่เปิด resource ใช้ compact resource launcher ไม่กินที่ preview เปล่า
...
- Header 46px และ spacing scale 4/8/12/16/24px เป็น starting tokens; controls desktop ≥32px hit area, touch ≥44pt; text UI 13–14px, transcript 14–15px/1.55–1.65, mobile body เริ่ม 16pt และรองรับ Dynamic Type. เป็นข้อเสนอที่ต้องตรวจ Thai clipping ไม่ใช่ reference measurement
- ต่อ semantic `--caret-*` กับ Code-OSS theme variables เดิม; light/dark/high-contrast. Radius controls 5px/cards 8px/composer 10px ตามฐานเดิมก่อนทดสอบ visual delta. ไม่ติดตั้ง design system ใหม่
...
1. **CX-01 blocked:** user-provided sanitized Codex screenshots/recording (Agent home, composer menus, running tools, approval, review/browser, settings) or a future permitted reference session. Do not bypass app safety restriction. Pixel freeze of Codex-derived surfaces waits for this evidence; behavior specification can proceed
